...
—— 關鍵字都有但文法壞掉會判「意思對了」，用了比參考答案更自然的說法反而可能判
「再想想」。所以按下「對答案」之後，除了原本的三級判定與參考答案，還會多一段
**模型看你那一句本身**的修正。
...
┃ 🤖 AI 改的
┃ Can I get a medium latte to go, please?   🔊    ← 貼著你原本的說法改，不是把參考答案抄過來
...
**參考答案照樣在，而且緊接著。** 它免費、離線、每次都一樣，而模型的意見每次不同
...
| 決定 | 為什麼 |
|---|---|
| **排在參考答案上面，但不取代它** | 兩者回答的是不同的問題：參考答案是「教材建議怎麼說」（寫死、每次一樣），AI 修正是「我這樣說行不行」。兩句並列才對照得出來 |
| 逐字比對、教材說明、AI 出處收進 `<details>` | 第一次看有用、第二十次是雜訊，而攤開來會把「下一題」擠到螢幕外面。刪掉的話想追究的人就沒地方追究了 |
| 本地批改**先**畫出來，AI 修正才去要 | 反過來的話整張結果卡要等模型回來才看得到，而那幾秒裡使用者什麼都沒有 |
| prompt 裡明講「不要把參考說法整句抄過來」 | 不講的話回來的永遠是參考說法本身，而那個畫面上已經有了 |
| 上下文都送出去 | 少了情境，模型只能就句子論句子（同一句話在咖啡店與在藥局是完全不同的評語）；中翻英少了中文題目更嚴重 —— 那時候連「他想講什麼」都不知道 |
| 失敗一律回 **200 + `ok:false`** | 本地批改與參考答案已經在畫面上了。把它變成紅色的錯誤，會讓人以為剛剛那次作答壞了 |
...
本地替代品 —— 本地做得到的（關鍵字比對、參考答案）畫面上本來就有了。
...
| 額度滿了時各模式**退的方式不一樣** | 發音評分回 429（沒有分數的話這次錄音什麼都做不了）；中文講評退回本地摘要（分數已經拿到了）；AI 修正回 200 + `reason: 'quota'`（本地批改與參考答案本來就在） |
...
| 情境對話的 AI 修正 | 對完答案就送 | 結果卡上留一顆按鈕 | 只有本地批改與參考說法 |
| 中翻英的 AI 修正 | 對完答案就送 | 結果卡上留一顆按鈕 | 只有本地批改與參考答案 |
...
| ✍️ 中翻英 | `⌘/Ctrl+Enter` 對答案・`Enter` 下一題・`N` 換一題・`S` 唸一次答案・`A` 讓 AI 看我這一句・`H` 提示 |
| 💬 情境對話 | `⌘/Ctrl+Enter` 對答案・`Enter` 繼續・`P` 再聽一次・`S` 唸參考說法・`N` 換一段・`A` AI 修正・`H` 看參考說法 |
...
│   │   │                       #（**情境對話與中翻英共用**）
│   │   ├── answer-lines.js    # 結果卡最上面的兩句並列：🤖 AI 改的在上、📘 參考答案在下
