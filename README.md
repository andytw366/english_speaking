...
| ✍️ 中翻英 | 2,159 題（填空 161 / 整句 1,998），每日進度與連續天數，**AI 修正你寫的那一句**（選用） |
| 💬 情境對話 | 93 段 / 651 句台詞（八個情境都有），角色扮演，每日進度與連續天數，**AI 修正你自己寫的那一句**（選用） |
| 🗣️ 跟讀 | 2,041 句，錄音後拿到**逐音素**的發音評估與**語調圖** |
...
- **model 不放進練習時的那個選單。** 那個選單是 Gemini 專屬的、是使用者每次練習
  ...
  它在設定頁的「AI 金鑰與模型」那張卡上（只有擁有者填得動），跟金鑰同一條路
  —— 手機上換模型不必 ssh 進伺服器。
...
設定頁的「AI 金鑰與模型」那張卡可以直接填，存了**立刻生效，不用重啟**。
可以改的是這七個變數：`AZURE_SPEECH_KEY`、`AZURE_SPEECH_REGION`、`GEMINI_API_KEY`、
以及那四個 `NARRATION_*`（另外還有兩個呼叫上限，見「每天最多呼叫幾次」）。前端永遠拿不到完整金鑰：
...
u/<userId>.json         學習進度（就是備份檔的那幾個鍵）
u/<userId>.rev-<n>.json 最近 10 個舊版本
```

不用資料庫是因為要存的東西就是 `BACKUP_KEYS` 那幾個鍵，一個使用者不到 1 MB
（`srs` 就算 10,040 個字全練過也才 0.57 MB，`activity` 與 `results` 兩張每日表
留滿 400 天也只有十幾萬 bytes）。
...
| 時機 | 為什麼是這個時機 |
|---|---|
| App 打開時 | 先把另一台練的東西合下來再開始練 |
| 切到背景時 | 手機上「切出去」跟「關掉」幾乎是同一件事，而 `visibilitychange` 是最後還來得及發請求的時機 |
| 有寫入之後（debounce 10 秒） | 練一輪單字卡會寫幾十次，每次都推太吵 |
| 回到前景時 | 另一台可能在這段時間練過 |

掛鉤的位置是 `lib/storage.js` 的 `write()` —— 那是**所有**寫入的唯一出入口
（`read` / `write` 是模組私有的，每個 export 都經過它），所以不必去改五個模式，
也不會有哪個模式漏掉。

**失敗什麼都不做，繼續練。** 離線、伺服器掛了都保持 dirty，下一個時機再試。
唯一會讓使用者看到東西的是 session 過期（401）—— 不重新登入的話永遠同步不上去，
那件事非講不可。

> **自動同步可以關掉**（設定 → 跨裝置同步 → 自動同步）。這是**每台裝置各自的選擇**，
> 所以存在自己一個 localStorage 鍵而不是 `settings` 裡 —— `settings` 會跟著同步，
> 放進去就變成「在一台關掉、每一台都關掉」。

### 合併規則

合併在**伺服器端**做（`POST /api/sync/merge`）：讀 → 合 → 寫在 store 的同一個
獨佔區段裡完成，所以不會有「讀完之後另一台先寫進去」而要重試的 409 迴圈。
而規則本身是 `public/lib/merge.js` 的**純函式，伺服器 import 同一份** ——
兩邊各寫一份的話一定會分岔，而分岔的症狀是「同步之後數字對不起來」，
沒有錯誤訊息。

每一條規則都必須滿足兩個性質，否則就是錯的：

| 性質 | 不成立會怎樣 |
|---|---|
| **冪等** `merge(merge(a,b), b) === merge(a,b)` | 重試一次數字就膨脹 |
| **交換律** `merge(a,b) === merge(b,a)` | 同步順序會影響結果，兩台裝置一直互相推翻 |

| 資料 | 規則 |
|---|---|
| `activity` | 每台裝置一格，**逐格取 max**（見下） |
| `results` | 形狀跟 `activity` 一樣，**逐格、逐計數器取 max**（計數器只會往上加） |
| `srs` | 每張卡整筆取 `at` 比較新的那一次 |
| `history` | 用 `at` + `sentenceId` 取聯集 |
| `settings` | 比 `updatedAt`，**整包**取比較新的那一邊 |
| `vocabDays` | 上一代的鍵，已經沒有寫入者，逐日取 max（只求不弄丟） |
...
所以形狀是 `{ '2026-09-07': { 'dev-a1b2': 3, 'dev-c3d4': 2 } }`：
寫入只加自己那一格、讀取把格子加起來、合併逐格取 max。
**同一格只有一台裝置會動它**，所以取 max 就等於取那台的最新值 ——
同時不少算也不膨脹。

`deviceId` 存在 localStorage，**不在 `BACKUP_KEYS` 裡**。還原到另一台裝置時
要是同一個 id，兩台從此互相覆蓋對方的格子 —— 而症狀是「明明兩台都練了，
數字卻只有一台的」，沒有任何錯誤訊息。

改版之前的資料（一天一個數字）搬進一個**固定的 `legacy` 格**，不是本機那一格 ——
兩台各搬一次的話，同一段歷史會被算成兩台的份而加倍。代價是同步之前的歷史
兩邊取比較大的那個（略為少算），而那些值只餵連續天數（看那一天有沒有練過），
今天的數字每天重新算。

#### 已知的限制

同一張卡在兩台裝置上、在兩次同步之間各答過一次時，`seen` / `correct`
會少算一次 —— 取的是整筆比較新的那個，不是把次數加起來。要完全正確得做
向量時鐘，對「一個人、偶爾換裝置」不划算。而 `box` 與 `due` 是對的，
那才是決定下次何時複習的欄位。

### 整包覆蓋還留著（逃生門）

「設定 → 跨裝置同步 → 整包覆蓋」收著兩顆按鈕。**理由是逃生門**：
合併規則萬一出問題，「用這一台的整份蓋過去」是使用者唯一能自己救回來的動作。
語意是覆蓋，所以每一次都會先把兩邊的內容並排出來讓你確認 ——
只問「確定嗎」等於沒問。

用 `rev`（每寫一次加一）做樂觀鎖：上傳時帶著你讀到的 `rev`，對不上就回 409
並把伺服器上的整包帶回來。沒有這道的話，兩台裝置幾乎同時上傳，
後到的會**無聲**蓋掉先到的。（自動合併那條路不需要樂觀鎖 ——
合併本身是冪等且滿足交換律的，讀到的 rev 過期了也不會有任何損失。）
...
首頁把已經有的資料放在一起：**目前的能力**（四個面向的雷達圖 + 現在最該練什麼）、
今天各模式練了多少 / 目標多少、連續天數，以及**有幾個字到期要複習**。
點任何一列就跳進那個模式。

主欄的順序是**今天總覽 → 能力量表 → 今天的目標**。量表排在目標清單前面，
是因為清單回答「今天還差多少」、量表回答「我該練哪一個」—— 先知道該練哪一個，
那份清單才知道要從哪一行開始看。
...
- **待複習的數字直接數 `srs` 裡到期的筆數**，不載入任何字庫檔（六個分級加起來
  3 MB）。首頁只需要一個數字。
- **能力量表也不載入任何字庫檔**：四個面向全部從每日成績表（`results`）、
  盒號（`srs`）與跟讀紀錄（`history`）算得出來。`test/ability.test.js` 有一條
  測試直接掃 `lib/ability.js` 的原始碼，擋的就是「有人為了算得更準而去 import 字庫」。
...
| 面向 | 怎麼算 | 哪裡來 |
|---|---|---|
| 🗂️ 字彙 | 練過的字現在**平均在第幾盒**（第 1 盒 = 0、第 6 盒 = 100） | `srs` |
| 🎧 聽得懂 | 最近的**題目**正確率（不是題組） | `results.listening` |
| ✍️ 表達 | 中翻英 + 情境對話的通過率 | `results.translation` / `results.dialogue` |
| 🗣️ 唸得準 | 最近的平均分（`sum / n`） | `results.shadowing` |

**為什麼是四個面向而不是五個模式**：中翻英與情境對話考的是同一件事 ——
能不能把想講的意思用英文說出來。一個用寫的、一個在對話裡，而「這句說得通嗎」
的判定本來就是同一套（`lib/verdict.js` 兩個模式共用）。分成兩軸的話，
同一種能力在圖上佔兩格，看起來像它比聽力重要兩倍。

**四個軸的單位不一樣，而這件事沒有被藏起來。** 聽得懂 / 表達 / 唸得準是比率或分數，
字彙是狀態。硬要湊成同一個單位做不到，所以：圖上四條都只講「熟不熟」（0～100），
**量**（練過幾個字、最近幾題）用文字寫在圖旁邊。只看圖的話，練熟 5 個字的人
字彙軸會是滿的 —— 那不是 bug，是這張圖本來就不回答「你會幾個字」。

**沒有資料要畫成「還沒練過」，不是 0。** 樣本不夠時分數是 `null`，那一軸畫成虛線、
標籤寫「—」，多邊形只連有資料的點。0 分的意思是「練了，很差」；
沒練過的模式畫成 0 等於對著沒碰過那一項的人說「你這裡是 0 分」。
門檻是「一個數字開始有意義的最小量」（聽力 / 表達 10 題、字彙 10 個字、跟讀 3 次）。

**「表達」優先採用模型的判定。** 本地那一份是關鍵字比對，它看不懂句子，
而且手寫的那批中翻英題有已知的 keywords 欠債（會把題目自己列的另一種說法判成錯）。
兩種判定混在一起平均，算出來的數字會同時受「你寫得好不好」與「這題的關鍵字挑得準不準」
影響 —— 而使用者只看得到前者的解釋。所以成績表把 `aiN` / `aiOk` 分開記，
模型判過的量夠了就用它，不夠才退回關鍵字比對，並且在畫面上寫明是哪一種。

> **圖底下那張「現在最該練什麼」才是回答「我目前的問題是什麼」的東西。**
> 雷達圖給的是一眼的高低，而「聽力那一格比較短」不是一個可以拿去做的結論；
> 「最近 20 題只對 11 題，去練聽力」才是。每一行都有一個具體的數字與一顆按鈕。
> 問題音（th、r / l…）併進「唸得準」那一行而不是另開一行 —— 那是同一件事的細節，
> 拆開的話畫面上會有兩行都在講發音，而使用者只有一個發音問題。

### 今天練完了的總結（五個模式共用）

達到今天的目標時，主欄換成一張總結：今天練了幾個、答對率 / 平均分、
新字幾個、連續天數，加上一句鼓勵與「接下來去哪」（指向目前最弱的那個面向，
**但排掉剛練完的這個模式**）。實作在 `lib/day-summary.js`。

**擋在動線上。** 跟讀的「一組練完」原本畫在講評下面的第四張卡，而那個位置
在手機上要再捲兩三個螢幕 —— 那張總結一次也沒被看到過。這一張是同一個道理，
只是尺度換成一天。

**擋在「下一個」上，不是在作答的當下**：剛對完答案最想看的是自己這一句差在哪、
AI 怎麼說，總結蓋上去等於把那一題白答了。所以觸發點是**換一題 / 換一句 /
換一段情境**那顆按鈕（單字卡例外，見下）。

| 模式 | 什麼時候出現 |
|---|---|
| 🗂️ 單字卡 | 就是它原本的「今天練完了」那一頁 —— **不是一次性的**，再進來還是看得到 |
| 🎧 聽力 | 達標之後的第一次「換一題」 |
| ✍️ 中翻英 | 達標之後的第一次「下一題」 |
| 💬 情境對話 | 達標之後的第一次「換一段情境」（對話中間插進來會打斷角色扮演） |
| 🗣️ 跟讀 | 達標之後的第一次「換一句」。**它會把「一組練完」那張吃掉**（見下） |

> **跟讀的兩張總結會撞在一起。** 每日目標預設 5 句、一組也是 5 句，所以
> 「這一組練完了」與「今天練完了」會在同一刻一起等著。連續彈兩張幾乎一樣的卡
> 比只彈一張差，所以今天那張補上了「最高 / 最低」，一張就講得完。
> 被吃掉的只有那一次 —— 第 6 句起「一組練完」照常接手。

**鼓勵只講事實，而且是具體的數字。** 這條規矩從 `today-card.js` 的 `todayNote()`
就開始了（刻意不寫「你今天還沒練，連續天數要斷了」—— 用罰的去推人回來，
短期有效，長期只會讓人不想打開）。這裡往前再一步：

```
✅ 今天的正確率 90%，比前 2 天的 50% 高了 40 個百分點。
✅ 今天的目標是 10 題，你練了 14 題，多做了 4 題。
❌ 太棒了！你真是太厲害了！繼續加油！
```

第二種講了等於沒講，而且每天都一樣 —— 看三次就變成一個要按掉的東西。
排序是「哪一件事最值得講」：進步 > 超出目標 > 連續天數 > 只是把事實講一次。
`test/ability.test.js` 有一條測試掃過這幾句話，確認裡面一個罰的字都沒有。

**「今天看過了沒」只存在這台裝置**（`summarySeen`，刻意不進備份與同步）：
它不是學習進度，是「這個畫面有沒有出現過」。同步的話，在手機上看過總結，
桌機打開就再也不會看到那一天的總結了 —— 而那兩台是兩個不同的當下。

### 🗂️ 單字卡

10,040 字。用 **Leitner 盒子制**而不是 SM-2 —— 行為好預測、出問題也容易看懂。

**複習規則**（規則本身是純函式，在 `lib/storage.js`，`vocabulary.test.js` 逐條釘住）：

| | 規則 | 為什麼 |
|---|---|---|
| 盒子 | 1～6 盒，間隔 **1 → 3 → 7 → 16 → 35 天**（第 1 盒是「今天之內」） | 最後一盒 35 天：真的記住的字不必每三週回來煩一次。第 6 盒＝「已熟練」 |
| 答對 | 往上一盒，間隔拉長 | |
| 答錯 | **退一盒**，不是掉回第 1 盒 | 練一個月爬到第 5 盒的字，一次手滑不該從頭來過 ——「一次答錯」跟「完全沒學過」不是同一件事。連錯兩次自然就退到第 1 盒，該從頭來的還是會從頭來 |
| 到期 | **以「日」為單位**：間隔 N 天 = N 天後那一天的 00:00，不是「現在再過 N×24 小時」 | 存成「＋24 小時」的話，今晚 23:00 答的字明天整個白天都還沒到期 —— 只要今天比昨天早一點打開 App，昨天的字就一個都不算到期，而畫面上完全看不出原因（「待複習 0，可是我昨天明明練了 20 個」） |
| 抽卡順序 | 到期的優先（越早到期越前面），再補沒學過的 | |
| 新字 | 另外一個每日上限（預設 10），**在今天的份裡先保留名額**，最多佔今天的一半 | 見下面 |

> **為什麼新字要有自己的額度。** 到期的字跟新字共用同一條隊伍的話，
> 兩種壞情況都會發生：到期的一多，連續好幾天一個新字都看不到（進度條動也不動）；
> 到期的是 0 的那幾天，又會一次灌進一整天份的全新字 —— 而今天發多少新字，
> 明天就要回來複習多少，複習量會忽多忽少。所以新字先保留名額，剩下的才給複習。
>
> **保留的名額最多是今天的一半**：複習仍然優先（欠的複習會生利息，拖一天就多一天的
> 遺忘），保留區只是不讓它把新字整個吃光。少了這條，「今天的份 3 個、新字上限 10 個」
> 會變成三張全是新字，到期的字一張都排不進來。
>
> 上限是**護欄不是鎖**：發完之後畫面會講清楚是上限擋住的（不是只說「練完了」），
> 而且那顆「再多練 10 個」同時多給 10 個新字的額度。
>
> **改版的影響**：原本在第 5 盒（舊的最後一盒）的字會從「已熟練」退回「學習中」，
> 再答對一次就回到第 6 盒。**刻意不自動把它們升上去** —— 那等於憑空給一個沒答對過的
> 間隔，而少算一次比多算一次安全。已經排好的到期時間一個都沒有動。
...
**「看複習盒」列出哪些字在哪個盒子**（側欄的「看複習盒」）：六個盒子各一張卡，
寫出間隔（1 天 → 3 天 → 7 天 → 16 天 → 35 天）、字數、幾個到期了，然後是字表 ——
...
設 0 表示不設目標，到期的字會一次全部排進來（新字仍然照上限發）。

> 每日目標旁邊還有一個「其中最多幾個新字」（預設 10）—— 今天的 20 個字裡，
> 最多 10 個是沒學過的，其餘留給到期要複習的。規則與理由在上面「🗂️ 單字卡」。
> 第一天因此只會發到 10 張（那時候還沒有東西可以複習），這是刻意的：
> 今天發多少新字，明天就要回來複習多少。

> **今天的最後一題答完之後，先把答案看完再切過去。** 成績是在**選下去的當下**
> 就記進去的（中途關掉 App，那一題本來就答完了，不該不算），所以答完最後一題時
> 「今天還剩幾個」立刻變 0 —— 畫面若只看這個數字就換卡，這一題的正確答案、
> 卡片背面與「其他選項」會被「今天練完了」整個蓋掉，最後一題等於白答。
> 所以那張完成卡多一個條件：**這張卡還停在「已作答、還沒按下一題」時不切**，
> 而且按鈕會從「下一題」改成「完成今天的份」，講清楚按下去會發生什麼事。
...
| 跟讀 | **一句算一次**。同一句今天錄第二次、第三次不重複算 —— 那是在把一句練好，不是又練了兩句（隔天再練同一句就是新的一句了） |
...
#### 第二張表：每天練得**怎麼樣**（`results`）

計數表只回答「今天練了幾個」。能力量表要的是「練得怎麼樣」，所以另外有一張
形狀一樣的表（模式 → 日期 → 裝置格），只是格子裡放的不是一個數字而是一組計數器：

```json
{ "listening": { "2026-09-18": { "dev-a1b2": { "n": 3, "q": 12, "ok": 7 } } } }
```

| 模式 | 計數器 |
|---|---|
| 單字卡 | `n` 答完幾張、`ok` 答對幾張、`fresh` 其中幾個是今天第一次見到的新字 |
| 聽力 | `n` 幾組、`q` 那幾組總共幾題、`ok` 答對幾題（**今天的份算組，正確率算題**） |
| 中翻英 | `n` 幾題、`ok` 通過幾題，其中 `aiN` / `aiOk` 是模型看過的那一部分 |
| 情境對話 | 同中翻英。**沒作答就不記**（走過那一句今天的份照算，但正確率的分母只能是真的寫過的句子） |
| 跟讀 | `n` 幾句、`sum` 分數總和（平均 = `sum / n`） |

三個設計決定：

- **兩張表分開，刻意的。** `activity` 是連續天數與每日目標的唯一來源，
  是這個 App 裡最不能弄壞的一份資料。混在同一張表的話，能力量表寫錯一行
  就會讓連續天數歸零。
- **每一個計數器都只會往上加**，這是合併能用「逐格取 max」的全部理由
  （規則直接沿用 `mergeActivity` 那一套）。**平均分存 `sum` 而不是 `avg`**
  就是為了這件事：平均會上上下下，取 max 會挑到那一天分數最高的**中間狀態**，
  而那個數字誰也沒看過。
- **判定要記「是誰判的」。** 中翻英與對話的 `aiN` / `aiOk` 分開存，
  理由見上面「四個能力面向」那一節。

> **成績跟著「今天的份」同一刻寫進去**（`lib/daily.js` 的 `recordPractice()`
> 一次寫兩張），不是各模式自己去呼叫第二個函式 —— 分兩個呼叫點的話，
> 遲早有一個模式只記了其中一張，而那種錯誤完全沒有徵兆：練習量正常累積，
> 只有能力圖悄悄少算。唯一的例外是中翻英的 AI 判定（`recordOutcome()`）：
> 今天的份在按下「對答案」那一刻就要加上去，而模型的判定兩秒後才回來。

> **「清除每日紀錄」會把兩張表一起清掉。** 只清一張的話，首頁上會同時出現
...
跟讀紀錄（`history`）、情境對話的 AI 修正（`reviews`）與偏好設定（`settings`）。
**不含金鑰** —— 那些在伺服器的 `.env`。
...
**但這三級現在是退路，不是結論。** 接得到模型的時候，結果卡上那一句判定是
AI 給的，題目附的答案降級成一句**例句**（見下一節）—— 關鍵字比對看不懂句子，
而題庫裡那一句只是「可以這樣說」的其中一種寫法。
...
本地批改看得懂的只有「關鍵字有沒有出現」。它免費、離線、瞬間，但**它看不懂你寫了什麼**
—— 關鍵字都有但文法壞掉會判「意思對了」，用了比題庫那句更自然的說法反而可能判
「再想想」。所以按下「對答案」之後，模型會看你寫的那一句本身，回一個修正加上
**這一題的判定**。

**判定是模型給的，題目附的答案只是一個例句。** 這兩件事是同一個決定的兩面
（`lib/verdict.js`）：同一個意思有幾十種講法，把題庫裡那一種當標準答案的話，
使用者寫出更自然的說法反而會被判錯。以前畫面上**同時有兩個判定** ——
最上面是本地關鍵字比對、下面一行是模型的 —— 而它們常常互相打臉：
...
使用者只能相信其中一個，而畫面沒有告訴他要相信哪一個。現在只有一句判定：

| 模型的狀態 | 最上面那一句 |
|---|---|
| 看過了（含存下來的那一份） | `✅ AI：這樣說可以` ／ `🟡 AI：小地方可以更自然` ／ `❌ AI：這句要改` |
| 正在看 | `🤖 AI 正在看你這一句…`（灰的 —— **不先給一個兩秒後會被推翻的結論**） |
| 沒看（關掉／手動還沒按／這次沒回來／額度用完） | 退回本地那三級，標題上不會有「AI」 |
| 空白作答 | `請先寫下答案`（空白不會去要修正，模型上一題的判定拿來用會是錯的） |

跟著判定一起改的還有兩處：**「少了關鍵用字：…」只在退回本地時才出現**
（判定是模型給的時候，那一行講的是「例句有而你沒寫的字」，跟結論無關），
而情境對話總結的「幾句表達到位」也改成看同一個判定（`isPass()`：模型判
「可以」與「小問題」都算到位）—— 同一句話在卡片上說可以、在總結裡算沒過關的話，
那個數字就沒有意義了。

**情境對話與中翻英是同一份程式**（後端 `server/coach.js`、前端 `lib/ai-review.js`）：
它們問的是同一個問題「我這樣寫，母語人士會怎麼說」，差別只有上下文
（一邊是情境與對方剛剛那句，一邊是中文題目與題型；填空題還會把句型一起送去，
因為「grab」單獨看不出對錯）。各寫一份的話，換模型之後你分不出
「對話變好、翻譯變差」是模型的差別還是兩份 prompt 的差別。

##### 兩句並列：AI 改的在上，例句在下

結果卡最上面是**兩句並列**（`lib/answer-lines.js`，兩個模式共用）：

```
❌ 再想想

┃ 🤖 AI 改的
┃ Can I get a medium latte to go, please?   🔊    ← 貼著你原本的說法改，不是把例句抄過來

┃ 📘 例句
┃ Could I have a medium latte to go?        🔊    ← 教材寫死的那一句

• 點餐時用 Can I get… 比 I want… 客氣自然。
• 美式咖啡店說 to go，take away 是英式用法。

▸ 看詳細比對          ← 逐字比對、教材說明、「由哪個模型產生、等了幾秒」都收在這裡
```

**為什麼 AI 那句排在上面。** 使用者剛剛寫了一句話，他要的是「那到底該怎麼說」，
而最貼近他寫的那一句的答案是模型改出來的那一句 —— 教材那句例句回答的是
「這題的標準說法」，是另一個問題。原本 AI 那段接在整張卡的**最後面**，
要先捲過逐字比對、其他說法、教材說明才看得到，而那幾段每一題都一樣。

**例句照樣在，而且緊接著。** 它免費、離線、每次都一樣，而模型的意見每次不同
也可能出錯 —— 兩句擺在一起才對照得出來。兩行的左邊各有一條色帶（AI 是強調色、
教材是灰色，判定不同顏色也會跟著換）：混在一起的話，看不出哪一句該信到什麼程度。

AI 那一行**五種狀態各說各的話**（還沒要／正在要／要到了／這次沒回來／伺服器沒設定
模型），因為分不清楚的代價都是「以為壞了」。設成「關」的時候整行不畫 ——
那正是「關」跟「手動」的差別。
...
| 決定 | 為什麼 |
|---|---|
| **排在例句上面，而判定聽它的** | 兩句回答的是不同的問題：例句是「可以這樣說」的一種寫法（寫死、每次一樣），AI 修正是「我這樣說行不行」。兩句並列才對照得出來，而「這一題算好算壞」只有後者答得出來 |
| 判定**只有一句**（`lib/verdict.js`） | 兩個判定擺在同一張卡上會互相打臉，而畫面沒辦法告訴使用者該信哪一個。挑哪一個是純函式，五種狀態都測得到 |
| 等模型的那幾秒給**中性**的標題 | 先秀本地判定的話，「❌ 再想想」會閃一下再變成「✅ 這樣說可以」—— 使用者記得的是先看到的那一個 |
| 逐字比對、教材說明、AI 出處收進 `<details>` | 第一次看有用、第二十次是雜訊，而攤開來會把「下一題」擠到螢幕外面。刪掉的話想追究的人就沒地方追究了 |
| 結果卡**先**畫出來，AI 修正才去要 | 反過來的話整張卡要等模型回來才看得到，而那幾秒裡使用者什麼都沒有。等的只有最上面那一句判定，例句、逐字比對、按鈕都立刻在 |
| prompt 裡明講「不要把例句整句抄過來」 | 不講的話回來的永遠是例句本身，而那個畫面上已經有了 |
| prompt 裡也明講「例句不是標準答案」 | 不講的話模型會拿它當標準，把每一種跟它不同的說法都判成「要改」—— 那正是本地比對會犯的錯，而現在模型的判定就是結論，錯了沒有第二道關可以擋 |
| 上下文都送出去 | 少了情境，模型只能就句子論句子（同一句話在咖啡店與在藥局是完全不同的評語）；中翻英少了中文題目更嚴重 —— 那時候連「他想講什麼」都不知道 |
| 失敗一律回 **200 + `ok:false`** | 判定會退回本地三級，而例句本來就在畫面上。把它變成紅色的錯誤，會讓人以為剛剛那次作答壞了 |
| 供應商沿用 `narrator.js` 那一份 | 「現在接得到哪個模型」的邏輯只能有一份。跟講評共用 `NARRATION_*` 設定，換模型時兩邊一起換 |
| 自動／手動／關 | 這是**唯一「打字就花錢」的地方**。三個 AI 功能用的是同一組選項（見下面「AI 設定集中在一起」）—— 手動模式下結果卡上留一個按鈕（或按 `A`），按了才呼叫 |

`NARRATION_PROVIDER=local`（或什麼都沒設定）時它**不能用**，而且畫面上會直接寫出
缺什麼。注意這跟講評不一樣：講評在 local 之下還有本地摘要可以退，AI 修正沒有
本地替代品 —— 本地做得到的（關鍵字比對、例句）畫面上本來就有了。

**修正會存下來，而且同一句話不會問第二次。** 存在 `localStorage` 的 `reviews`
（鍵是「哪一段對話的第幾句」，值裡帶著當時寫的那句話）：

- 重新整理不會消失，而且對話記錄裡那幾句自己說過的話下面會多一行「🤖 更自然：…」
- **句子一樣就直接拿存下來的，不再呼叫一次**。「再試一次」按下去、答案一個字
  都沒改是很常見的動作，而每一次都是一次要花錢的呼叫。句子改過了就重新問
  —— 舊的那份講的是另一句話，拿出來會變成「AI 說的跟我寫的對不上」
- 進備份也進同步（`BACKUP_KEYS` / `SYNC_KEYS`），合併規則是「每一格取比較新的」。
  它是**花錢換來的**，換一台裝置就重新付一次的話這份快取等於只在原本那台上有用
- 設定 → 學習資料看得到有幾筆，也清得掉（清掉的代價會寫在確認視窗裡）


#### 每天最多呼叫幾次

加了 AI 修正之後，這個 App 第一次有了「**打字就花錢**」的地方 —— 跟讀還算得出
上限（一句一次錄音，人手速有極限），對話是每按一次「對答案」就一次呼叫。
所以有一個每人每天的上限，預設 **200 次**：

```bash
AI_DAILY_LIMIT=200                      # 全部模式、全部服務加起來。off = 不限制
AI_DAILY_LIMITS=gemini=50; azure=off    # 同一個預算裡，各個模型再有自己的上限
```

（兩個都可以從網頁設：**設定 → 每天的呼叫上限**，只有擁有者改得動。）

| 決定 | 為什麼 |
|---|---|
| **一個預算，不是每個模式各一個** | 花的錢是同一個帳戶的。「對話還有 30 次、跟讀只剩 2 次」這種狀態沒辦法解釋 —— 使用者在意的是「我今天還能練多少」 |
| Azure 的發音評估也算一次 | 它一樣是花錢的呼叫。上限要能擋住的是「今天總共花了多少」 |
| **不同的模型可以有不同的上限** | 它們的價錢與配額差好幾個數量級（Gemini 免費層算請求數、HF 的 Inference Providers 算 token、Azure 免費層算音訊時數）。只寫供應商名稱（`gemini` / `openai` / `azure`）就是那條路的所有 model 一起算 |
| 兩道都要過 | 總量與該模型各自的上限，任一個滿了就擋。擋下來的兩種原因**講不同的話** —— 一種是「今天練夠了」，另一種是「換個模型還能繼續」 |
| 計數存在**伺服器**（`DATA_DIR/usage.json`） | 存在 `localStorage` 的話，換一台裝置、開個無痕視窗、或按一下「清除學習資料」就重新開始 —— 那不是上限，是提醒 |
| 扣在**呼叫之前** | 模型沒回來的那一次錢照樣花了。扣完再呼叫的話，超時的請求等於免費 |
| 判斷與加一在同一個獨佔區段裡 | 「先讀再寫」中間會讓出去，兩個幾乎同時進來的請求會各自讀到同一個舊數字然後都放行 —— 上限就變成「大約」 |
| 額度滿了時各模式**退的方式不一樣** | 發音評分回 429（沒有分數的話這次錄音什麼都做不了）；中文講評退回本地摘要（分數已經拿到了）；AI 修正回 200 + `reason: 'quota'`（判定退回本地三級，例句本來就在） |

畫面上：剩 20 次以內時，AI 修正那一塊會多一行「今天還可以呼叫 N 次」——
剩很多的時候不寫，不然一個安全網會變成一個計時器。設定頁那張卡**每個帳號都
看得到今天用了幾次**（分模型列出來），因為「今天的 AI 修正怎麼不見了」在看不到
數字的情況下完全無從判斷：額度用完、金鑰壞了、還是網路不通，三件事的畫面幾乎一樣。

### 🗣️ 跟讀

這個模式是整合的落點：錄音 → Azure 逐音素評分 → 中文講評 → 語調圖，
而且練習紀錄會回頭決定下一句抽什麼。整套規則在下面的
**「🗣️ 跟讀：抽下一句、語調圖、一組 5 句」**（在說明頁那一節後面），
無人聲怎麼擋在更後面的**「無人聲偵測」**。

### ⚙️ 設定

八張卡，順序是「先講這台機器會做什麼，再講它拿什麼做的，最後才是我的資料」：

| 卡 | 內容 | 誰看得到 |
|---|---|---|
| 🤖 AI 功能 | 三個會呼叫模型的功能各自**自動／手動／關**，＋ 現在接得到哪個模型、今天用了幾次 | 每個帳號 |
| 🧠 AI 金鑰與模型 | 最上面是「來源」與 Gemini 的 model，接著是現在真的接得到什麼，然後三組金鑰各自**收在摺疊裡**（Gemini／OpenAI 相容端點／Azure） | 內容只有擁有者，狀態每個帳號都看得到 |
| 每天的呼叫上限 | 總量與逐模型的上限、今天分別用了幾次 | 每個帳號看得到，擁有者改得動 |
| 每日目標 | 五個模式各一個數字 | 每個帳號 |
| 練習偏好 | 情境、難度、單字卡題型、中翻英題型、聽力自動播放、跟讀抽句 | 每個帳號 |
| 語音 / 跨裝置同步 / 學習資料 | TTS、帳號與同步、備份與清除 | 每個帳號 |

每張卡的小字都縮到一句話，細節指到 ❓ 說明頁（見下面「小字寫到什麼程度」）。

#### AI 設定集中在一起

原本這幾件事散在三張卡上：跟讀的講評開關在「練習偏好」裡（旁邊是情境與難度的
篩選）、情境對話的修正在它下面、而「現在走哪個模型」在另一張卡。
但使用者心裡它們是**同一件事**：這個 App 什麼時候會去花錢、花多久。

金鑰與模型也合成一張（原本是「AI 金鑰」＋「講評與修正用的模型」兩張）——
它們回答的是同一個問題「這台伺服器拿什麼在做 AI」，分兩張的代價是換一個模型
要在兩張卡之間來回，而「填了金鑰卻沒生效」時也看不出是哪一張的問題。
卡內的順序按**多久會動一次**排：

1. **來源與 model** —— 換模型是會反覆調的事（想快一點、想省一點），所以放最上面。
   來源是一排 chip（四個值，按下去直接存），不是下拉
2. **現在真的接得到什麼** —— 改完馬上要確認的那一行
3. **三組金鑰，各自收在 `<details>` 裡** —— 填一次就不再看的東西，攤開來只會
   擋住上面那兩件事。預設狀態是「**沒設定的展開、設定好的收起來**」：全新安裝時
   三組都攤開（有事要做），設好之後自己變乾淨。使用者手動開關過的狀態會記住到
   離開設定頁 —— 每次重畫都重算的話，存完一次就被收起來，而接下來多半正要改
   同一區的另一格（剛存完 Azure 金鑰要填區域）
4. 三個儲存按鈕，一區一個 —— 合成一顆的話「已儲存」出現時，使用者不知道剛剛
   到底送出了哪幾個欄位（金鑰欄是密碼欄，畫面上看不出它有沒有被動到）

而且三個功能現在用**同一組選項**：

| | 自動 | 手動 | 關 |
|---|---|---|---|
| 跟讀的中文講評 | 送出錄音時一起要 | 先看分數，按「要中文講評」（或 `A`）才要 | 一律本地摘要 |
| 情境對話的 AI 修正 | 對完答案就送 | 結果卡上留一顆按鈕 | 判定退回本地比對，只剩例句 |
| 中翻英的 AI 修正 | 對完答案就送 | 結果卡上留一顆按鈕 | 判定退回本地比對，只剩例句 |

**為什麼三個都要有「手動」而不是只有開／關**：這些是唯一會花錢、也唯一要等的
功能，而「今天先不自動，但這一句我真的想知道」是最常見的狀態 —— 開／關表達不出來。
講評的手動模式為此多了一個端點（`POST /api/narration`）：它收的是**已經算好的
分數**，不是重送錄音（重送等於再花一次 Azure 的錢）。

舊設定會自己搬過來：`geminiNarration: false` → 「關」（那時候沒有手動），
`dialogueAiReview: false` → 「手動」（那時候關掉之後按鈕還在，那就是手動）。

#### 同一種選擇，長同一個樣子

控制項收在 `public/lib/fields.js`，規則只有一條：**選擇的形狀決定控制項**，
而不是「這個畫面剛好想用什麼」。

| 形狀 | 控制項 |
|---|---|
| 幾個互斥的值（開／關、自動／手動／關、只練填空／只練整句） | `chipField` —— 一排會亮起來的 chip，按下去就生效 |
| 可複選（情境、難度、題型） | `multiChipField` |
| 要打字的（金鑰、網址、model id） | `textField` ＋ 一顆「儲存」 |
| 很長的清單（語音、model 白名單） | `selectField`（唯一用得上 `<select>` 的地方） |

修掉的不一致：跟讀的「優先練弱點」原本是模式裡的一個 checkbox（而別處同樣的
選擇都是 chip）—— 看到 checkbox 的人會去找「儲存」按鈕；跟讀的每日目標原本是
下拉，而它只有四個值 —— 下拉要多按一下才看得到自己有哪些選擇。
兩個都換成跟設定頁一樣的 chip，而且**留在原地**（在跟讀畫面上就能改）。

順帶修掉一個沒接線的設定：`autoPlayListening` 在 `DEFAULTS` 裡放了很久，
但沒有任何地方讀它，設定頁上也沒有 —— 現在它真的會讓聽力換一題就自動播，
而且在「練習偏好」裡選得到。

#### 小字寫到什麼程度

設定頁一度背著整份說明書。每一段小字單獨看都有道理（「這個選項選了會怎樣」
「這份資料清掉會怎樣」），但加起來的結果是**一半的像素在講解**，而真正要按的
控制項被擠得很散。更糟的是那些字**每次進來都在**：第一次讀有用，第二十次只是噪音。

規則改成一條：

> **留下「現在要做這個決定時需要知道的事」，一句話講完。其餘搬到 ❓ 說明頁。**

| 留下 | 搬走 |
|---|---|
| 「自動＝每次都要　手動＝按了才要　關＝不呼叫」 | 三種模式各自的完整行為與花費差異 |
| 「所有模式加在一起算。」 | 額度怎麼扣、存在哪、換裝置會不會重算 |
| 「這兩顆是**覆蓋**不是合併：其中一邊會完全取代另一邊，無法復原。」 | 同步的合併規則與階段設計 |
| 「這些存在這個瀏覽器裡，清除瀏覽資料就會消失。」 | 備份檔裡有什麼、怎麼帶到新裝置 |

**按鈕會造成不可逆後果的那幾句一個字都沒縮**（覆蓋、清除、登出）——
那些不是說明，是按下去之前唯一要知道的事。

每張卡最下面留一顆「看說明」（`linkbtn`）指到說明頁的那一題。整段搬走卻不留
指路的話，那些字對使用者等於不存在。實測設定頁的高度：桌機 3,578 → 3,089 px、
手機 6,008 → 5,246 px（−13%），而少掉的全部是文字。

同一條規則也套到別的畫面：逐字比對的圖例從一句話變成「紅＝你沒寫到　灰＝你多寫的」、
首頁的複習排程從兩句變一句、頁尾那段麥克風與 `localStorage` 的說明整段收進說明頁
（它原本出現在**每一個**畫面的最下面，而真正需要它的時機只有兩個）。

---

## ❓ 說明 / Q&A 頁

上一節那些搬走的字的去處。`public/modes/help.js`，五張卡、21 組問答：
開始用 ／ AI 修正與講評 ／ 進度與資料 ／ 怪怪的時候 ／ 鍵盤。

三個決定：

- **排版是 `<details>` 問答，全部收起來，標題就是問題本身。**
  攤開來寫成長文的話，這一頁會變成另一個沒有人讀的地方 —— 而使用者來這裡
  永遠是帶著一個具體的問題（「AI 修正怎麼沒出現」「要換電腦了怎麼辦」）。
- **副標題寫「不用讀，需要的時候再來翻」。** 這一頁不是入門教學，
  它是設定頁與練習畫面**卸下來的重量**。當成必讀的話等於什麼都沒省到。
- **不進手機下方那一列。** 那一列一格放得下一個圖示加兩三個中文字，五格剛好；
  多一格就開始擠成兩行。跟設定一樣走頁首的按鈕（❓ 在 ⚙️ 左邊），
  而桌機的左側模式列兩個都列得出來。

入口有三個：桌機左側模式列、頁首的 ❓、以及每一頁頁尾的「有問題？看說明」。
設定頁另外會直接指到相關的那一題。

問答的內容盡量從程式裡長出來而不是再抄一份 —— 模式清單讀 `MODES`、
全域快捷鍵讀 `GLOBAL_KEYS`，不然加一個模式就會有一份說明悄悄過期。

---

## 🗣️ 跟讀：抽下一句、語調圖、一組 5 句

一句練完之後，這個模式做四件事：**決定下一句抽什麼**、**畫出你的語調**、
**記今天練了幾句**、**把中文意思擺在句子下面**。這一節照這個順序講。

（送出錄音那一段 —— 兩組金鑰、兩條講評路徑、無人聲怎麼擋 —— 分別在
「兩組金鑰，各自負責不同的事」與「無人聲偵測」兩節。）
...
看的是那一句的**紀錄分數 —— 同一句練很多次時，算數的是最高的那一次**
（`recordScore()`）。
...
> 一句唸到 95 分了，平均還停在 62，於是它照樣被判定成「你這句很爛」而一直被抽出來，
> 而句子旁邊的 chip 上寫的也是 62。發音是「做得到就是做得到」，
> 最高分才是真正的能力上限。
>
> 代價講清楚：僥倖的一次好成績會留著。可以接受 —— 抽句還有下面②那一半，
> 所以那一句只是變罕見，不會消失。
>
> 平均分數沒有消失，只是換了位置：它還在「練習紀錄」那張卡的統計裡
> （那裡算的是**每一次**練習，本來就該是平均），以及句子旁邊的「練過 N 次」。
...
複習間隔由分數決定（同樣是紀錄分數 —— 最高的那一次），**每差 25 分就差一倍**：
...
- 句子旁：`練過 3 次・最高 82 分・10 天前・該複習了`（到期那段是琥珀色）
...
Azure 的 prosody 只給一個分數，回答不了「**我的語調長什麼樣子**」。而語調是少數
**看懂一張圖就馬上知道要改什麼**的東西：句尾該降的地方你升上去了，一眼就看得出來，
不需要任何術語。

所以送出錄音之後，講評上面多一張圖：**你自己那一句的基頻曲線**（實線），
以及**同一句由 Azure 唸出來的語調**（淡色虛線），字標在下面。

```
你的語調：起伏明顯（高低差約 8 個半音）。　淡色那條是範例。　Azure 點名的字：nearest
```

那行字是**圖的替代文字**：看不到圖的人（螢幕閱讀器、圖沒載出來）要靠它知道
發生了什麼事，所以它得自己講得完整。

怎麼算的（`lib/pitch.js`，純函式）：

| 決定 | 為什麼 |
|---|---|
| **YIN 的簡化版**（累積平均正規化的差分函數），不是裸的自相關 | 自相關在低頻會被「整數倍週期也很像」騙走 —— 症狀是曲線突然跳 12 個半音，看起來像使用者唱破了。殘餘的再用 `fixOctaveJumps()` 掃一遍 |
| **降到 8 kHz 再算** | 計算量剩約四分之一（三秒的錄音大約 30 毫秒），而 400 Hz 以下完全夠用。先做一次很輕的低通再抽樣，不然摺回來的高頻會讓差分函數多出假的低點 |
| **在 `blobToWav()` 裡算**，跟音量特徵同一個地方 | 那是唯一手上有 PCM 的地方。把 `Float32Array` 往外傳就要有人負責在對的時機放掉它 |
| **逐字位置來自 Azure 的 `Words[].Offset/Duration`** | 沒有它 x 軸只是「第幾秒」，看不出「是哪個字唸高了」。Gemini 那條路沒有逐字資料，就只畫曲線 |

這張圖**刻意不做**的三件事：

- **不算分數。** Azure 已經給 prosody 分數了；這裡再算一個「語調分」，
  兩個數字遲早會對不起來，而使用者不知道該信哪一個。這張圖是拿來**看**的。
- **不把無聲的地方連起來。** 子音與停頓本來就沒有音高，連一條線上去看起來像
  「你這裡拖了一個長音」—— 那是圖在說謊。切段的規則是 `contourRuns()`。
- **不比絕對音高。** y 軸是半音、以**這一句自己的中位數**為 0。男聲低、女聲高，
  比絕對值沒有意義；要比的是形狀（哪裡升、哪裡降、起伏多大）。

> **演算法的邊界寫在測試裡**：高於 400 Hz 的純音會被算成它的次諧波
> （800 Hz 會抽出 267 Hz），因為那個週期也在搜尋範圍裡而且完全吻合。
> 朗讀的基頻不會到那裡，所以可以接受 —— 但別拿這份程式去分析唱歌或樂器。
> `pitch.test.js` 有一條就是在釘這件事，免得下一個人把它當成 bug 去「修」。

#### 範例那一條線

淡色那條得來不易。難點不在畫圖，在於原本的示範發音是瀏覽器的 `speechSynthesis` ——
那個 API 沒有任何辦法把唸出來的聲音接回程式裡（沒有 MediaStream、也進不了
Web Audio），所以拿不到音訊就抽不出曲線。只能另外合成一份。

| 決定 | 為什麼 |
|---|---|
| **懶產生**：按下「開始錄音」或「播放正確發音」才去要 | 句庫 2,041 句而你實際會練到的大概一兩百句，全部先算等於先付二十倍的錢。**但也不能等到送出錄音**：那時候使用者已經在等分數了，而且範例曲線在**錄之前**看到更有用。按下錄音之後有三秒可以用，剛好把延遲藏起來。**尤其不是「換一句」就要** —— 那是這個模式裡按得最兇的按鈕 |
| **自己一支 `GET /api/reference-pitch/:id`**，不塞進評分那一支 | 評分那條路不能被第二個外部呼叫拖下水（TTS 慢了、掛了，分數照樣要回來）；GET + 固定 id 還能讓 service worker 快取，練過的句子離線也看得到 |
| 存 `DATA_DIR/pitch/<音色>/<id>.json`（`store.js` 的獨佔佇列 + 原子寫入），音訊是同一個資料夾裡的 `.wav` | 一句一個檔：寫入是懶產生的，一個大檔要整份讀改寫，而且兩台裝置同時練不同句子會互相覆蓋。**音色在路徑裡**：換了音色曲線就整個不一樣，混在一起的話圖上會出現「這句是甲的語調、那句是乙的」。磁碟：一句約 100 KB，只有真的練過的句子才會產生（練 300 句大約 30 MB） |
| **不進備份、也不進同步** | 這是這個 store 裡唯一跟使用者無關的東西：一句話的參考曲線是**那句話**的屬性，而且弄丟了可以再產生一次。備份與同步那兩份是「弄丟就沒了」的資料 |
| 額度**只有快取沒中才扣**（走既有的 `spendQuota`） | 讀檔不花錢。扣不到就是沒有範例曲線 —— 分數與講評完全不受影響 |
| 抽曲線的程式**伺服器 import 前端那一份** | `public/lib/pitch.js` 是純函式，`lib/merge.js` 已經是同樣的先例。兩套程式算出來的曲線會有系統性差異，而那看起來就像「我唸得不像」 |

**兩條線怎麼對齊**：逐字對齊，不是整句按比例拉。你唸得比範例慢（或某個字拖得
特別長）時，整句等比例縮放會讓後面每一個字都錯開 —— 圖上看起來像你整段語調都不對，
而其實只是節奏不同。兩邊都有逐字時間（你的來自 Azure 的 `Words[].Offset/Duration`、
範例的來自合成時的 `wordBoundary` 事件），所以對得起來。

> **字數對不上就退回整句按比例。** 漏字、多唸、或 Azure 少認了一個字都會讓兩邊的
> 字數不同，這時候硬對會把第 3 個字對到第 5 個字上 —— 那比按比例還糟。
> 退回去至少整體形狀還是對的，而「漏字」本來就有別的指標在講（完整度）。

> **任何一種失敗都只是「這次沒有範例曲線」**：沒設金鑰、額度用完、合成失敗、
> 連不上 —— 一律回 200 加 `ok: false`，前端就只畫自己那一條。
> 這張圖是加分的，它掛掉不該讓畫面出現紅色的錯誤。

#### 示範發音放的也是同一份音訊

合成出來的 WAV 跟曲線存在一起（`pitch/<音色>/<id>.wav`），
「🔊 播放正確發音」優先播它 —— 不然圖上畫的是 Azure 的語調、耳朵聽到的是
瀏覽器內建的聲音，**兩個人的語調本來就不同，使用者會以為圖畫錯了**。

- 播哪一個由 `demoSource()` 決定（純函式）。沒存到音訊、還沒產生、或瀏覽器播不出來，
  就退回瀏覽器的 TTS —— 跟以前完全一樣。那個函式還擋一件事：
  **換過句子之後不會放上一句的音訊**（那種 bug 聽起來一切正常，
  只是唸的不是畫面上那句話）。
- **第一次按會等一下下（最多 1.2 秒）**：那時候通常還在合成。等不到就先用 TTS 出聲，
  下一次按就會是 Azure 的了 —— 乾等三秒才聽到聲音，比聽到一個不同的聲音更糟。
- `GET /api/reference-audio/:id` **只讀快取、不合成**：產生是
  `/api/reference-pitch/:id` 的事（那一支才扣額度），沒有就 404。

> **金鑰的檢查在快取之後。** 金鑰拿掉、或換一台沒設金鑰的機器掛同一個 volume 時，
> 已經產生過的曲線與音訊照樣給 —— 沒金鑰只代表「產生不出新的」。
> 這條是實際踩到的：一開始把檢查放在最前面，結果有快取也拿不到。
...
- **一組 5 句**：「換一句、再換一句」是沒有終點的，很容易練兩句就關掉。練完給一份總結：
  ...
  一組 5 句指的是 **5 個句子**：同一句重錄不會讓 `3 / 5` 變成 `4 / 5`，
  而是換掉那一句在這一組裡的成績（留分數高的那一次，跟紀錄分數同一條規則）。

> 「今天練了幾句」也是**一句算一次**（同一天同一句錄三次算一句），
> 規則與理由在「每日進度與連續天數（五個模式共用）」那張表。

#### 總結擋在「換一句」前面，不是畫在講評下面

原本它是主欄的第四張卡（句子 → 錄音 → 講評 → 總結），而那個位置在手機上要再捲
兩三個螢幕 —— 實際用起來就是一路按「換一句」，那張總結一次也沒被看到過。

現在一組練完之後，「🔀 換一句」那顆按鈕變成「✅ 這一組 5 句練完了，看總結」，
按下去**整個主欄換成總結那一頁**（側欄的今天與練習紀錄照舊留著），
上面那顆「再練一組（5 句）」接回下一句。只多一次點擊，而且是在使用者
本來就要按的那顆按鈕上。

**不自動彈出來、也不插在講評上面**：剛錄完最想看的是自己這一句幾分，
總結是「這一組」的結論，等他要往下走的那一刻才是它的位置。

> 判斷寫成純函式 `nextSentenceAction()`（`practice.js`）—— 它是一個狀態機，
> 而它錯掉的方式不會有錯誤訊息：兩個判斷的順序寫反的話，看完總結按下一句
> 會再進總結一次，永遠出不去（寫的時候真的踩到過，鍵盤 `N` 那條路）。
...
40 次「移動滑鼠 → 瞄準 → 點」，而鍵盤是 20 次按鍵。

**有哪些鍵有兩個地方看得到**：左側模式列上常駐前四個（跟著模式換），
按 `?` 叫出完整的一張表（這個模式的全部 + 全域的）。
快捷鍵最大的問題不是難按，是沒人知道有這個東西 —— 側欄那幾行負責「讓人知道有」，
`?` 那張表負責「一次看完」。

### 一套共通的語言

同一個鍵在每個模式都做同一類的事，不然換一個模式就要重學一次：

| 鍵 | 意思 |
|---|---|
| `Enter` | 這個畫面的**主要動作**（對答案／繼續／下一題／送出錄音） |
| 空白 | 有錄音的模式就是錄音，沒有就跟 `Enter` 一樣 |
| `N` | 換一題／一句／一段 |
...
各模式實際有哪些（沒有那顆按鈕的模式就沒有那個鍵）：

| 模式 | 鍵 |
|---|---|
| 🏠 今天 | `1`–`5` 跳到那個模式 |
| 🗂️ 單字卡 | `1`–`4` 選答案・`Enter`／空白 下一題・空白 翻卡・`S` 唸一次 |
| 🎧 聽力 | `P` 播放・`1`–`4` 作答・`Enter` 對答案／下一題・`N` 換一題・`T` 看原文 |
| ✍️ 中翻英 | `⌘/Ctrl+Enter` 對答案・`Enter` 下一題・`N` 換一題・`S` 唸一次例句・`A` 讓 AI 看我這一句・`H` 提示 |
| 💬 情境對話 | `⌘/Ctrl+Enter` 對答案・`Enter` 繼續・`P` 再聽一次・`S` 唸例句・`N` 換一段・`A` AI 修正・`H` 看例句 |
| 🗣️ 跟讀 | 空白 開始／停止錄音・`Enter` 送出・`P` 播放範例・`N` 換一句・`A` 要中文講評 |
| 不分模式 | `?` 快捷鍵表・`[` `]` 上／下一個模式・`Esc` 關閉說明 |

**單字卡刻意沒有 `N`**：它是一副有順序的牌組（到期的優先），「跳過這張」會讓
「今天練了幾張」對不起來 —— 畫面上沒有那顆按鈕，所以也不給那個鍵。

**換模式用 `[` `]` 而不是數字鍵**：數字在四個模式裡是「選第幾個選項」，
...
全域那幾個（`?` / `[` / `]` / `Esc`）綁在 `app.js`，而且**綁得比模式早** ——
它先看，沒接走的才輪到模式。

> 第 1 條也管到全域鍵：中翻英與情境對話一進去焦點就在輸入框裡，
> 這時候按 `]` 要打出一個 `]` 而不是換模式。所以在那兩個模式裡要用 `?`／`[`／`]`
> 得先讓焦點離開輸入框（或直接點側欄那一行「全部快捷鍵」）。
...
│   ├── audio.js               # WAV 解析：能量分析（有沒有人聲）＋ 解成 PCM（語調曲線用）
│   ├── tts.js                 # Azure 語音合成（範例句的語調曲線與示範發音）
│   ├── reference-pitch.js     # 範例曲線與音訊：懶產生、存起來、抽曲線用前端那一份 pitch.js
│   ├── narration.js           # 講評開關 + 沒用 Gemini 時的本地摘要（純函式）
│   ├── coach.js               # 情境對話的 AI 修正：prompt 與解析（純函式）＋ 呼叫
│   ├── quota.js               # 每天最多呼叫幾次（純函式；計數存在 store.js）
...
│   │   ├── grade.js           # 中翻英的前端批改（**沒有 AI 時的退路**）
│   │   ├── verdict.js         # 結果卡最上面那一句判定是誰說的：有模型就聽模型的
│   │   │                       #（同上，兩個模式共用；純函式）
│   │   ├── ai-review.js       # AI 修正與手動講評：能不能用、送出、狀態機、那一段畫面
│   │   │                       #（**情境對話與中翻英共用**）
│   │   ├── answer-lines.js    # 結果卡最上面的兩句並列：🤖 AI 改的在上、📘 例句在下
│   │   │                       #（同上，兩個模式共用；細節收進 <details> 的 moreBox 也在這）
│   │   ├── fields.js          # 設定用的控制項（同一種選擇一律長同一個樣子）
│   │   ├── recorder.js        # 麥克風與 MediaRecorder
│   │   ├── wav-encoder.js     # 錄音 → 16 kHz 單聲道 WAV + 音量分析 + 語調曲線
│   │   ├── pitch.js           # 從 PCM 抽基頻（YIN 簡化版，純函式，**之後伺服器 import 同一份**）
│   │   ├── pitch-chart.js     # 語調圖（inline SVG）＋兩條曲線的逐字對齊
│   │   ├── reference-pitch.js # 去伺服器要「範例句」的曲線與音訊（什麼時機要、播哪一個）
│   │   ├── waveform.js        # AnalyserNode 即時波形
│   │   ├── tts.js             # 示範發音
│   │   ├── modes.js           # 模式登錄表：標籤、圖示、副標題、每日進度的單位、快捷鍵
...
│       ├── settings.js
│       └── help.js            # 說明 / Q&A：設定頁與練習畫面卸下來的那些小字
└── test/                      # 見「測試」
```
...
| GET | `/api/capabilities` | `{ azureConfigured, geminiConfigured, narration, aiReview, quota, usage }` —— 要登入。設定頁「目前」那一行讀它，情境對話讀 `aiReview` 決定要不要顯示 AI 修正；`usage` 是**這個帳號**今天用掉的次數 |
...
| | | 今天的次數用完時回 **429**（分數拿不到就沒有東西可看）；只有講評那一段的額度滿了則照常回分數，`narrationReason` 是 `quota` |
| POST | `/api/answer-review` | AI 修正，**情境對話與中翻英共用**。JSON：`mode`（`dialogue` / `translation`）+ `input`（你寫的句子，必填）+ 各自的上下文。回 `{ ok, review: { corrected, verdict, notes }, label, ms, quota }`。**沒設定模型、呼叫失敗、今天的次數用完，一樣是 200**，帶 `ok:false` 與 `reason`（`no_key` / `failed` / `quota`） |
| POST | `/api/narration` | 手動要一次中文講評（跟讀的「手動」模式）。JSON：`assessment`（**已經算好的分數**，不是錄音 —— 重送錄音等於再花一次 Azure 的錢）。回 `{ ok, feedback_zh, label, ms, quota }` |
...
| `reference-pitch.test.js` | 範例句的曲線：**合成器是注入的假的**（不呼叫 Azure、不花錢），音訊是自己合出來的正弦波。釘住快取命中不扣額度、沒中才合成而且存起來、額度扣不到就不合成、存不進去不算失敗（曲線已經算出來了），**沒金鑰時已經有的快取照樣給**（金鑰只決定「產生不出新的」）、音訊存不進去時曲線要寫 `hasAudio: false`（反過來會留下一份說謊的曲線）、以及**兩條線的對齊**（逐字對齊；字數對不上退回按比例 —— 硬對會把第 3 個字對到第 5 個字上）。另外釘住 `demoSource()`：有音訊就放 Azure 那一份，**換過句子之後不會放上一句的**（那種 bug 聽起來一切正常，只是唸的不是畫面上那句話） |
| `pitch.test.js` | 語調曲線（F0）。合成音測準確度（固定頻率、掃頻、降到 8 kHz 分析不會跑掉），真人錄音（`fixtures/speech-16k.wav`）測「像不像真的」。另外釘住三件壞掉時不會有錯誤訊息的事：**停頓那幾格是 `null` 不是插值**（補一條假的線等於在圖上說謊）、**孤立的八度跳會被拉回來**（圖上一根 12 個半音的刺，使用者會以為自己唱破了）、以及**真正的語調起伏不可以被壓平**。還有一條記著演算法的邊界：高於 400 Hz 的純音會被算成次諧波，**那不是 bug** |
| `practice.test.js` | 間隔重複、弱點音加權、連續天數、一組總結、**紀錄分數取最高的那一次**（`recordScore()`）、**今天這一句練過了沒**（`practisedOn()`，「一句算一次」的判斷）、以及**按下「換一句」該做什麼**（`nextSentenceAction()`，總結擋在前面的那個狀態機）。跑 3,000 次抽樣驗機率分布，**一律注入 `now`** —— 這些函式全部跟時間有關，用真實時鐘的話測試會在半夜跑的時候紅一次、隔天自己又好了 |
...
| `ability.test.js` | 每日成績表與四個能力面向。每一段算錯的症狀都是**靜悄悄的**：畫面照樣畫得出一張圖、一個分數，只是那個數字不對。所以規則一條一條釘：計數器只會往上加（合併規則要的就是這個性質）、**樣本不夠時是 `null` 不是 0**（「沒練過」不等於「0 分」）、聽力的正確率照題算不照組算、表達優先採用模型的判定、白名單外的欄位會被丟掉。另外兩條是規矩而不是計算：**`lib/ability.js` 不可以碰字庫檔**（直接掃原始碼，擋的是「為了算得更準而 import 3 MB 的字庫」）、以及**鼓勵的那幾句話裡一個罰的字都沒有** |
| `merge.test.js` | 兩份進度怎麼合成一份 —— **整個跨裝置同步裡最不能出錯的一段**，因為合併寫錯是靜悄悄的（使用者只會看到「昨天練的怎麼不見了」）。除了逐條規則，重點是每一種資料都各驗一次**冪等**與**交換律**；最後一條用固定種子隨機生 300 組兩台裝置的狀態去掃邊界。那條真的抓到一個 bug：`history` 用「先到的贏」處理識別相同但內容不同的紀錄，導致 `merge(a,b) ≠ merge(b,a)` |
...
| `settings.test.js` | 設定的搬家規則。搬家錯了**沒有任何錯誤訊息** —— 使用者設過的 50 無聲變回 20，或者一個數字的單位換了、數字沒換算，目標從此永遠達不到。聽力的目標從「題」變成「組」那次全部釘住：換算過、不會把小目標變成 0、0（不設目標）維持 0、靠旗標所以只換算一次、**只碰使用者設過的值不碰預設值**（`migrate()` 收併好 DEFAULTS 的版本就會把預設值一起除下去，真的踩過）。後來多了三個 AI 開關搬到同一組選項（自動／手動／關）：`geminiNarration:false` 要變成「關」、`dialogueAiReview:false` 要變成「手動」（兩者不對稱，因為那時候一個有按鈕、一個沒有），而且**巢狀的 `ai` 要在提早 return 之前併好** —— 漏掉的話所有舊資料的三個開關會通通吃預設值 |
...
把假的練習紀錄塞進 `localStorage`，驗八個分頁都載入得起來、**首頁的今天總覽**
...
**複習盒**（六個盒子、清單的字數跟側欄的「已熟練 / 學習中」對得起來、到期的標出來）、
**單字卡的每日目標**（練滿之後關掉重開不會重來、換難度不會歸零、
**今天最後一題答完先看得到答案**再切到完成卡）、
**複習規則**（答錯退一盒而不是掉回第 1 盒、間隔算到那一天的 00:00、
新字的每日額度不會被到期的字吃掉、額度用完時講得出是上限擋住的）、
...
對話裡對方的台詞不算你的練習量）、
**跟讀的計數與紀錄分數**（同一句錄三次只算一句、chip 上寫的是最高分）、
**語調圖**（停頓把曲線切成兩段而不是連起來、Azure 點名的字標出來、資料太少就不畫；
【9b】那一段用假麥克風真的錄一次，驗「錄音 → 抽曲線 → 畫圖」整條線接得起來）、
**一組 5 句練完**（用假麥克風真的錄五次，驗總結擋在「換一句」前面、自己占一頁、
「再練一組」接得回去）、**範例語調曲線**（按了錄音就去要、畫成第二條線、
對齊到自己的節奏，以及**按播放會去拿範例音訊而不是用瀏覽器的 TTS**）、清除紀錄。
