...
**下一步（還沒做）**：疊上「範例」那一條曲線。難點不在畫圖，在於現在的示範發音是
瀏覽器的 `speechSynthesis`，**拿不到音訊** —— 要另外用 Azure TTS 生一份。
設計（懶產生 + 伺服器快取 + 前後端共用同一份 `pitch.js`）寫在 `TODO.md`。
