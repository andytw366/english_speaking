...
「這句在練哪些音」、用語料詞頻自動分難度。新的句子附帶**中文意思**。

階段 10 把情境從 4 種擴到 **8 種**、每種 230～280 句，句庫合計 **2,041 句**。
面試類語料裡本來就只有 95 句，所以那一類**自己手寫**（`data/interview.txt`），
走同一套清洗與自動標音。
...
| 情境 | 句數 | | 情境 | 句數 |
|---|---|---|---|---|
| 日常對話 `daily` | 278 | | 學習 `school` | 250 |
| 旅遊 `travel` | 272 | | 健康 `health` | 250 |
| 職場 `work` | 263 | | 購物 `shopping` | 250 |
| 餐飲 `food` | 250 | | 面試 `interview` | 228 |

每種 250 句左右大約是「每天練 5～10 句、兩三個月不會重複」。
候選池還有一萬多句沒收 —— 沒有全收是因為每一句都會被抽到，多不等於好。

情境選單是從資料長出來的，所以加新情境只要在
`scripts/import-sentences.mjs` 補一組關鍵字、在 `public/labels.js` 補一行中文標籤。
...
Tatoeba 的面試類句子只有 **95 句**（其他情境都上千）—— 通用語料本來就不會有
「What does success look like in the first three months?」這種東西。
而這一類剛好是最該有品質的：會練它的人是真的要去面試。

所以那一類自己寫，放在 `data/interview.txt`（一行一句，`#` 開頭是註解），
匯入時走**同一套清洗與自動標音**：太長太短、有數字、查不到發音、
或是「整句都是很平的音、沒有練習重點」都會被擋下來並印出原因：

```
⚠ interview.txt：只有 4 個字，太短　「That's a good question.」
⚠ interview.txt：沒有明顯的練習重點（都是很平的音）　「I'm happy to be here today.」
```

內容涵蓋自我介紹、經歷、技能、團隊與衝突、弱點與失敗、離職原因、
未來規劃、薪資條件、反問面試官、行為面試、技術面試、電話與線上面試、收尾。

要自己加句子也走這條路：在 `data/` 放一個 `<情境>.txt` 就會被收進去。
...
講完就沒有下文，跟階段 6 之前那份唯讀的練習紀錄是同一個問題。

所以 `sentences.json` 的每一句多了 `focus`，標明它在練哪些音：
...
第三項是 1～2 倍：把最近 30 筆紀錄裡的 `issue` 統計起來，
一句話的 `focus` 涵蓋你越多問題，倍率越高。**下限一樣是 1 而不是 0** ——
練不到你弱點的句子只是不被偏好，不會被排除。全部只餵弱點音會讓練習變得很窄，
而且 `focus` 是人工標的，本來就不會完美。

只看最近 30 筆是刻意的：一年前就改掉的問題不該一直綁住現在的練習。

**看得見**：抽到練得到你弱點的句子時，旁邊會出現「這句在練 th 音」的綠色 chip
（綠色是因為它講的是「這句對你有用」，不是「你又錯了」—— 紅色留給實際唸錯的字）。
一組練完的總結也會接上一句「接下來會多抽一些練得到這些音的句子」，
把「告訴你問題」跟「針對問題出題」這兩件事連起來。

### 句庫：27 → 81 句

原本 27 句，配上間隔重複與一組 5 句之後大概兩三天就會開始重複。
新增的句子不是隨便湊數，是**照音去補的** —— 每個 `issue` 類型都要有夠多句子可以抽，
不然「多給你 th 的句子」會變成「一直給你同樣那三句 th」。
另外多了一個情境 `work`（職場日常，跟面試不一樣：視訊、進度、交件）。

| 練到的音 | 句數 |
|---|---|
| `r_l`（r 與 l） | 39 |
| `th`（θ/ð） | 28 |
| `plural_ed`（字尾 -s／-ed） | 21 |
| `v_w`（v 與 w） | 15 |
| `final_consonant`（字尾子音） | 13 |
| `n_ng`（字尾 n 與 ng） | 13 |
| `vowel_length`（長短母音） | 10 |
| `stress`（重音） | 9 |
| `linking`（連音） | 8 |
| `extra_vowel`（字尾多母音） | 6 |

情境選單是從資料長出來的，所以加 `work` 這個新情境不用改前端 ——
只有中文標籤要在 `public/labels.js` 補一行。
...
**七、支援其他 AI 供應商**（會需要第二組金鑰與另一套錯誤分類，錯誤處理的面積會變兩倍）。
