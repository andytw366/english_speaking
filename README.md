...
| 存 `DATA_DIR/pitch/<音色>/<id>.json`（`store.js` 的獨佔佇列 + 原子寫入） | 一句一個檔：寫入是懶產生的，一個大檔要整份讀改寫，而且兩台裝置同時練不同句子會互相覆蓋。**音色在路徑裡**：換了音色曲線就整個不一樣，混在一起的話圖上會出現「這句是甲的語調、那句是乙的」 |
...
**還沒做的**：示範發音（「🔊 播放正確發音」）仍然是瀏覽器的 TTS，所以**聽到的跟
看到的不是同一個人**。要一致的話得把合成出來的音訊也存下來（一句約 100 KB）
並改成播那一份 —— 留在 `TODO.md`。
...
│   ├── tts.js                 # Azure 語音合成（只為了算範例句的語調曲線）
│   ├── reference-pitch.js     # 範例曲線：懶產生、存起來、抽曲線用前端那一份 pitch.js
...
│   │   ├── reference-pitch.js # 去伺服器要「範例句」的曲線（什麼時機要寫在裡面）
...
| `reference-pitch.test.js` | 範例句的曲線：**合成器是注入的假的**（不呼叫 Azure、不花錢），音訊是自己合出來的正弦波。釘住快取命中不扣額度、沒中才合成而且存起來、額度扣不到就不合成、存不進去不算失敗（曲線已經算出來了），以及**兩條線的對齊**（逐字對齊；字數對不上退回按比例 —— 硬對會把第 3 個字對到第 5 個字上） |
