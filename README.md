...
所以送出錄音之後，講評上面多一張圖：**你自己那一句的基頻曲線**，字標在下面。

```
你的語調：起伏明顯（高低差約 8 個半音）。　Azure 點名的字：nearest
```
...
圖上還有第二條淡色虛線：**同一句由 Azure 唸出來的語調**。

難點不在畫圖，在於原本的示範發音是瀏覽器的 `speechSynthesis` ——
...
**示範發音放的也是同一份音訊。** 合成出來的 WAV 跟曲線存在一起
（`pitch/<音色>/<id>.wav`，一句約 100 KB），「🔊 播放正確發音」優先播它 ——
不然圖上畫的是 Azure 的語調、耳朵聽到的是瀏覽器內建的聲音，
**兩個人的語調本來就不同，使用者會以為圖畫錯了**。

> 哪一個聲音由 `demoSource()` 決定（純函式）。沒存到音訊、還沒產生、或
> 瀏覽器播不出來，就退回瀏覽器的 TTS —— 跟以前完全一樣。
>
> **第一次按會等一下下（最多 1.2 秒）**：那時候通常還在合成。等不到就先用 TTS
> 出聲，下一次按就會是 Azure 的了 —— 乾等三秒才聽到聲音，比聽到一個不同的聲音更糟。
>
...
> **總結擋在「換一句」前面，不是畫在講評下面。** 原本它是主欄的第四張卡
> （句子 → 錄音 → 講評 → 總結），而那個位置在手機上要再捲兩三個螢幕 ——
> 實際用起來就是一路按「換一句」，那張總結一次也沒被看到過。
>
