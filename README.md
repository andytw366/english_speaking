...
| `practice.test.js` | 間隔重複、弱點音加權、連續天數、一組總結、**紀錄分數取最高的那一次**（`recordScore()`）與**今天這一句練過了沒**（`practisedOn()`，「一句算一次」的判斷）。跑 3,000 次抽樣驗機率分布，**一律注入 `now`** —— 這些函式全部跟時間有關，用真實時鐘的話測試會在半夜跑的時候紅一次、隔天自己又好了 |
...
對話裡對方的台詞不算你的練習量）、
**跟讀的計數與紀錄分數**（同一句錄三次只算一句、chip 上寫的是最高分）、清除紀錄。
