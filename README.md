...
│   │   ├── grade.js           # 中翻英的前端批改
│   │   ├── ai-review.js       # 情境對話的 AI 修正：能不能用（快取一份）＋ 送出一句
...
| GET | `/api/capabilities` | `{ azureConfigured, geminiConfigured, narration, aiReview }` —— 要登入。設定頁「目前」那一行讀它，情境對話讀 `aiReview` 決定要不要顯示 AI 修正 |
...
| POST | `/api/dialogue-review` | JSON：`input`（你寫的句子，必填）+ 情境／角色／意圖／參考說法。回 `{ ok, review: { corrected, verdict, notes }, label, ms }`。**沒設定模型或呼叫失敗一樣是 200**，帶 `ok:false` 與 `reason` |
