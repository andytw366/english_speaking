...
| 情境、角色、對方剛剛那句都送出去 | 少了情境模型只能就句子論句子，而同一句話在咖啡店與在藥局是完全不同的評語 |
...
| 可以切成手動 | 這是**唯一「打字就花錢」的地方** —— 設定頁的「情境對話的 AI 修正」切到「手動」之後，結果卡上留一個按鈕，按了才呼叫 |
...
中文講評的開關、**情境對話的 AI 修正要自動還是手動**、講評用的 model、
TTS 語音與語速、學習資料的清除。
...
│   │   ├── grade.js           # 中翻英的前端批改
│   │   ├── ai-review.js       # 情境對話的 AI 修正：能不能用（快取一份）＋ 送出一句＋還剩幾次
...
| POST | `/api/dialogue-review` | JSON：`input`（你寫的句子，必填）+ 情境／角色／意圖／參考說法。回 `{ ok, review: { corrected, verdict, notes }, label, ms, quota }`。**沒設定模型、呼叫失敗、今天的次數用完，一樣是 200**，帶 `ok:false` 與 `reason`（`no_key` / `failed` / `quota`） |
...
| `coach.test.js` | 情境對話的 AI 修正：prompt 帶不帶得到情境與參考說法、模型各種不照格式回時解析得出什麼（markdown 圍欄、粗體、全形冒號、把「判定」寫成一段散文）、以及送上來的東西怎麼擋（空白答案是唯一的 4xx，其餘過長一律截掉）。**每一種不照格式在畫面上都是同一個症狀**（「AI 修正沒出現」），只有測試分得出是哪一種 |
...
| `reviews.test.js` | 情境對話 AI 修正的本機快取。重點是快取那一半：同一格是覆蓋不是留兩份、超過上限丟掉的是**時間**最舊的（同步合併之後鍵的順序不再是時間順序）、壞掉的值當成沒有、以及它真的在 `BACKUP_KEYS` 裡（漏掉的話換一台裝置就要重新付一次錢） |
