...
- **輸入式**（真的用打的）。選擇題已經涵蓋了主動回想，打字唯一多出來的是拼寫。
  要做的話中→英才有意義（英→中打中文批不出來），而且要處理 29% 同義撞號：
  提示首字母＋字數／接受同級的同義字／給一顆「其實我對了」。`lib/grade.js` 可以重用。
- **每日目標現在是一個數字，複習與新字共用**。到期的字超過每日目標時，那一天會
  全部拿去複習、抽不到新字（Anki 是拆成「每天新字」與「每天複習」兩個上限）。
  字量還不大時碰不到，真的遇到再拆。
- ECDICT 的 10,000 個字**沒有例句**（只有精選 40 字有），所以「句子挖空」這種題型
  目前只能用在精選牌組。要全部都有例句得離線生成一次。
