...
| ⚙️ 設定 | 金鑰與**講評端點都能從網頁設**（只有擁有者，存在 `DATA_DIR/settings.env`）、中文講評開關、**情境對話 AI 修正的自動／手動**、model、練習範圍、語音、學習資料、**跨裝置同步** | 完成 |
| 🔐 帳號 | 全部 `/api` 都要登入；進度存在伺服器上，**跨裝置自動合併**（手動整包覆蓋留著當逃生門） | 完成 |
| 🤖 LLM 即時對話 | 沒有腳本的自由對話 —— **只有規劃，還沒實作**（見「第四階段」） | 規劃 |

驗證狀態：`npm test` 412 項全過、`npm run test:ui` 240 項全過、
...
還沒做、如果之後要往那邊長的：修正的結果沒有存起來（重新整理就沒了），
也沒有「每人每天幾次」的上限 —— 現在是一句一次呼叫，而聊天是每一句都要。
