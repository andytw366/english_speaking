...
| 🗣️ 跟讀 | 2,041 句 / 8 種情境，Azure 逐音素評分 + **語調圖**（你自己那一條，範例那一條還沒做）+ 間隔重複 + 弱點音加權 + 連續天數（**一句算一次，同一句取最高分當紀錄**；一組練完的總結**擋在「換一句」前面**） | 完成 |
...
**階段 2（範例那一條，還沒做）** —— 難點不在畫圖，在於示範發音是瀏覽器的
`speechSynthesis`，**拿不到音訊**。討論後決定的做法：

- **懶產生**：使用者按「開始錄音」或「播放正確發音」時才去要（不是按「換一句」
  就要 —— 那會為根本沒練的句子付錢），用 Azure TTS 生一份、抽曲線、存起來。
- **自己一支 `GET /api/reference-pitch/:id`**，不要塞進 `/api/pronunciation-feedback`：
  評分那條路不能被第二個外部呼叫拖下水；GET + 固定 id 還能讓 service worker 快取，
  練過的句子離線也看得到。
- 存 `DATA_DIR/pitch/<voice>/<id>.json`（用 `store.js` 的獨佔佇列＋原子寫入）。
  它是**可再生的衍生資料**，所以**不進備份也不進同步**的白名單。
- 音色與語速是快取鍵的一部分；順便存 `wordBoundary` 給的逐字時間（對齊要用）。
- 額度只有 cache miss 才扣（走現有的 `spendQuota(req, {provider:'azure'})`），
  扣不到就是沒有範例曲線，**分數照常**。
- 可選的第三步：示範發音也改播 Azure 的同一個音色 —— 不然聽到的跟看到的不是同一個人。
