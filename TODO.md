...
**還沒做的（階段 3，可選）**：示範發音仍然是瀏覽器的 TTS，所以**聽到的跟看到的
不是同一個人**。要一致得把合成出來的音訊也存下來（一句約 100 KB，只存真的練過的
句子）並改成播那一份。`server/tts.js` 已經回了音訊 buffer，`reference-pitch.js`
現在只是用完就丟 —— 要加的話是「多存一個檔 + 一支 `GET /api/reference-audio/:id`」，
資料結構不用改。
