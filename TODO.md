...
（這一節是**快照**：現在有什麼、驗到什麼程度。改了什麼、為什麼那樣改在下一節
「做過什麼」；還沒做的在「待辦」。）
...
| 模式 | 內容 | 狀態 |
|---|---|---|
| 🏠 今天 | **四個能力面向的雷達圖 + 「現在最該練什麼」**、各模式今天的進度、連續天數、待複習數，點一下跳進去 | 完成 |
| 🗂️ 單字卡 | 10,040 字，**自己選難度**（6 級）＋各級進度＋**每日目標**（另含新字的每日上限）＋**中英雙向選擇題**（答完會列出其他選項的意思與發音），**Leitner 六個盒子**（1/3/7/16/35 天，答錯退一盒，到期以「日」為單位）＋**看複習盒**（哪些字在哪個盒子） | 完成 |
...
| ✍️ 中翻英 | **2,159 題**（填空 161 / 整句 1,998）＋每日進度＋**AI 修正**（跟情境對話同一份；**判定由 AI 給，題目附的答案只是一句例句**） | 完成 |
| 💬 情境對話 | 93 段 / 651 句台詞（八個情境）＋每日進度＋**AI 修正你寫的那一句**（選用，走 `NARRATION_*` 那條路；結果卡最上面是 🤖／📘 兩句並列，判定與總結都聽 AI 的） | 完成 |
| 🗣️ 跟讀 | 2,041 句 / 8 種情境，Azure 逐音素評分 + **語調圖**（你的 vs 範例，逐字對齊）+ 間隔重複 + 弱點音加權 + 連續天數（**一句算一次，同一句取最高分當紀錄**；一組練完的總結**擋在「換一句」前面**） | 完成 |
...
驗證狀態：`npm test` 604 項全過、`npm run test:ui` 330 項全過、
...
每一則都是「改了什麼 + 為什麼這樣改」，設計細節一律指到 README。
**由新到舊**，所以最上面那則就是這個 repo 現在的樣子是怎麼來的。

### 能力量表：第二張每日表、四個面向的雷達圖、每個模式的今日總結

起點是一個資料問題：**五個模式裡只有兩個存得了「練得怎麼樣」**。
單字卡有盒號、跟讀有逐筆分數，聽力 / 中翻英 / 情境對話只有「練了幾個」——
畫面上明明有「答對 4 / 6 題」，關掉就沒了。所以「各維度的能力」這件事，
不加一層資料就只做得出「各模式練了幾個」，而那就是首頁原本的樣子。

做了三件事：

**一、新的一張每日成績表（`results`）。** 形狀跟 `activity` 一模一樣
（模式 → 日期 → 裝置格），格子裡放的是一組**只會往上加**的計數器。
規則與每個模式記什麼在 README「第二張表：每天練得怎麼樣」。
三個關鍵決定：

- **不動 `activity`。** 它是連續天數與每日目標的唯一來源，混在同一張表的話，
  能力量表寫錯一行就會讓連續天數歸零。
- **計數器單調遞增**，所以合併直接沿用 `mergeActivity` 的「逐格取 max」。
  **平均分存 `sum` 不存 `avg`** 就是為了這件事（取 max 會挑到一個誰也沒看過的
  中間狀態）。`test/merge.test.js` 的冪等 / 交換律兩條性質照樣要過。
- **中翻英與對話把 AI 判定與本地判定分開記**（`aiN` / `aiOk`）。本地那份是
  關鍵字比對，而手寫的 118 題有已知的 keywords 欠債 —— 混在一起算出來的正確率
  會同時受「你寫得好不好」與「這題關鍵字挑得準不準」影響。

**二、首頁的四個能力面向（雷達圖 + 「現在最該練什麼」）。**
四個面向而不是五個模式：中翻英與情境對話考的是同一件事，分成兩軸會讓
同一種能力在圖上佔兩格。理由與每一軸怎麼算在 README「四個能力面向」。
**沒有資料畫成「還沒練過」而不是 0** —— 0 分的意思是「練了，很差」。
圖底下那張排行榜才是回答「我目前的問題是什麼」的東西，圖只給一眼的高低。

**三、每個模式「今天練完了」的總結**（`lib/day-summary.js`）。
擋在**「下一個」那顆按鈕上**，不是在作答的當下 —— 剛對完答案最想看的是自己
這一句差在哪，總結蓋上去等於把那一題白答了（單字卡的 `!picked` 條件早就是
同一個坑）。鼓勵**只講事實而且是具體的數字**，延續 `today-card.js` 的規矩。

順手修掉的：**`backupSummary()` 讀不懂現在的計數表形狀**，所以「確定要用這份
覆蓋嗎」底下那句摘要對任何一份現在的備份都說「每日紀錄 0 天」。它只認得舊的
一天一個數字，而計數表在做跨裝置同步時就改成一天一格了 —— 測試的 fixture
也還是舊形狀，所以一路沒被抓到。

**刻意沒做的是「最近的走勢」**（四個面向隨時間的變化）。規劃寫在待辦的
「能力量表的下一步」那一節 —— 資料層已經夠了，但滾動幾天、沒練的那幾天怎麼畫，
沒有兩三週的真實資料就是瞎猜，而聽力／中翻英／對話的成績是從這一版才開始累積的。

### 跟讀的語調圖：你的 + 範例，示範發音也是同一個人

送出錄音之後，講評上面多一張你這一句的基頻曲線，字標在下面，
Azure 點名語調有問題的字（Monotone／UnexpectedBreak／MissingBreak）標成琥珀色。
規則與取捨寫在 README「🗣️ 跟讀」那一節的「語調圖」。三個要點：

- 抽 F0 的是 `lib/pitch.js`（YIN 的簡化版，純函式，降到 8 kHz 再算，
  三秒的錄音約 30 毫秒）。**寫成純函式是為了之後伺服器 import 同一份** ——
  兩套程式算出來的兩條曲線會有系統性差異，而那看起來就像「我唸得不像」。
- 在 `blobToWav()` 裡算，跟音量特徵同一個地方：那是唯一手上有 PCM 的地方。
- 順手把 Azure 的 `Words[].Offset/Duration` 映出來（本來就在回應裡，只是沒取），
  x 軸才標得出字。

**階段 2 也做完了：圖上多了範例那一條淡色虛線。** 規則與理由在 README
「語調圖 → 範例那一條線」那一小節，一句話版本：按下「開始錄音」或「播放正確發音」時
才去 `GET /api/reference-pitch/:id` 要，伺服器第一次用 Azure TTS 合成一份、
抽曲線、存 `DATA_DIR/pitch/<音色>/<id>.json`，之後都是讀檔（不花錢、不扣額度）。
兩條線**逐字對齊**（字數對不上就退回整句按比例），所以比的是形狀不是快慢。
任何一種失敗都只回 `ok: false`，前端就只畫自己那一條。

**階段 3 也做完了：示範發音放的就是圖上那個人。** 合成出來的 WAV 跟曲線存在一起
（`pitch/<音色>/<id>.wav`），「🔊 播放正確發音」優先播它；沒存到、還沒產生、
或播不出來就退回瀏覽器的 TTS。第一次按最多等 1.2 秒（那時候通常還在合成），
等不到就先用 TTS 出聲 —— 乾等三秒比聽到一個不同的聲音更糟。
`GET /api/reference-audio/:id` **只讀快取、不合成**。

實作時踩到一個值得記住的：**金鑰的檢查一開始放在快取前面**，結果「有快取但沒金鑰」
也拿不到曲線。金鑰只決定「產生不出新的」，已經有的東西沒有理由消失 ——
換一台沒設金鑰的機器掛同一個 volume 就會遇到。`reference-pitch.test.js` 釘住了。

### 單字卡的複習規則改四條，跟讀的計數也跟著改

**單字卡的複習規則改了四條**（規則與理由都在 README「🗂️ 單字卡」，
...
同一輪還改了**跟讀的計數與紀錄分數**（規則在 `lib/practice.js`，都是純函式）：

- **一句算一次。** 原本是送出一次錄音就加一次，所以卡在同一句反覆重錄的那一天
  反而是數字最漂亮的一天。現在同一天的同一句只算一次（隔天再練就是新的一句），
  判斷用 `practisedOn()`；`buildActivity()` 把舊紀錄搬進計數表時也照同一條算，
  不然搬過來的舊數字會比新的膨脹。
- **同一句取最高分當紀錄**（`recordScore()`）。重錄是在把一句練好，用平均的話，
  願意重錄的人反而被自己前幾次的爛分數綁著 —— 唸到 95 分了，畫面還寫 62，
  而且照樣被判定成「你這句很爛」而一直被抽出來。抽句的分數權重與複習間隔
  都改讀它，句子旁邊的 chip 也從「平均 N 分」改成「最高 N 分」。
  平均沒有消失，它還在「練習紀錄」那張卡（那裡算的是每一次練習，本來就該是平均）。
- **一組 5 句指的是 5 個句子**：同一句重錄不會讓 `3 / 5` 變成 `4 / 5`，
  而是換掉那一句在這一組裡的成績（留分數高的那一次）。
- **一組練完的總結改成擋在「換一句」前面。** 它原本是主欄的第四張卡
  （句子 → 錄音 → 講評 → 總結），而那個位置在手機上要再捲兩三個螢幕 ——
  實際用起來就是一路按「換一句」，那張總結一次也沒被看到過。現在那顆按鈕會變成
  「✅ 這一組 5 句練完了，看總結」，按下去整個主欄換成總結那一頁，
  上面的「再練一組」接回下一句。判斷寫成純函式 `nextSentenceAction()`
  —— 它是狀態機，順序寫反的話看完總結按下一句會再進總結一次，永遠出不去。
...
**版面改版**（三個 commit）。桌機上原本只用掉 47% 的寬度、
...
把「一輪最多幾張」換成**每日目標**（設定裡的「單字卡每天練幾個字」，預設 20）：
...
> **當時的起點**（2026-09 盤點時，這張表解釋了下面的排序）：六個模式裡只有兩個有
> 「進度」—— 聽力一組答完會顯示「答對 4 / 6」，但**關掉就沒了**；中翻英連答對幾題
> 都沒算；對話重新整理就從第一句重來。
>
> | 模式 | 存進度 | 每日目標 | 連續天數 |
> |---|---|---|---|
...
> | 💬 情境對話 | ❌ | ❌ | ❌ |
>
> **這張表現在全部是 ✅**（1-b 做完了），留著是因為它是這份路線圖的理由。
...
rate limit 後來也做了：`server/quota.js` 一個**跨模式共用的每日預算**
（發音評分、中文講評、AI 修正、範例語調曲線全部記在同一個計數裡），
可以再對個別模型設自己的上限。計數存在伺服器（`store.spendUsage()`，
判斷與記錄在同一個獨佔區段裡，不然兩個幾乎同時進來的請求會各自讀到舊數字）。
設計寫在 README「每天最多呼叫幾次」。
...
按「對答案」之後多一段模型看你那一句的修正 —— 教材那句照樣在（現在叫「例句」），
...
4. **AI 改的那句排到例句上面**，兩句並列（`public/lib/answer-lines.js`，
...
6. **判定改成 AI 給的，題目附的答案降級成一句「例句」**（`public/lib/verdict.js`）。
   在這之前同一張結果卡上有**兩個判定**：最上面是本地關鍵字比對、下面一行是模型的，
   而它們常常互相打臉（「❌ 再想想」配「🤖 AI：這樣說可以」）—— 使用者只能相信
   其中一個，而畫面沒有告訴他要相信哪一個。現在挑哪一個是一個純函式：
   模型看過就聽模型的、正在看給一句中性的話（不先給一個兩秒後會被推翻的結論）、
   沒看才退回本地三級。跟著一起改的：「少了關鍵用字」只在退回本地時出現、
   情境對話總結的「幾句表達到位」改用同一個判定（`isPass()`，ok + minor 算到位）、
   prompt 裡明講「教材那句只是一個例句，不是標準答案」（不講的話模型會拿它當標準，
   把每一種跟它不同的說法都判成要改 —— 那正是本地比對會犯的錯）

### 第四階段：跟 AI 即時對話，自己指定情境與角色（**規劃，還沒實作**）
...
| 情境對話（現有） | 有標準答案的**一句** |
| 跟讀（現有） | 發音，逐音素的客觀分數 |
| **AI 對話（新）** | 沒有標準答案的**一段** —— 反應速度、接話、追問 |

「即時」指的是**打字來回即時**（送出 → 幾秒內對方就回你），不是語音即時打斷。
語音在階段 C／D，理由見「非目標」。

⚠️ **這個功能會改變 App 的成本結構。** 現在花錢的地方（跟讀的講評、AI 修正、
Azure 評分）都是「一個動作一次呼叫」，而聊天是**每打一句就一次呼叫，還要把前面的
對話一起送上去**。粗估一段十輪的對話大約 4～6k 個 input token 加幾百個 output，
而如果每一句都重送全部歷史，成本會隨句數**平方**成長。
好消息是預算機制已經有了（`server/quota.js`，所有模式共用一個每日上限），
壞消息是它是**以「次」計價**的，而聊天一段就是十幾次 —— 見下面雷 3。

#### 目標

1. **綁情境與角色**，不做自由聊天。情境、對方是誰、你是誰，三件事都要指定
   （怎麼指定見下一節）
2. 對方的英文要**壓在使用者的程度**上。不壓的話學習者只會一直點頭
3. **講評事後給，不打斷** —— 打斷等於練不到流暢度，而且那是 N 倍的呼叫
4. 算進「今天」的進度 —— 會是**第六個會累積進度的模式**
   （`storage.js` 的 `MODE_IDS` 現在是五個）、**第九個分頁**
   （`modes.js` 的 `MODES` 現在是八個：首頁 + 五個練習 + 設定 + 說明）
5. **離線時要一眼看得出它不能用**，不是壞在半路

#### 情境與角色怎麼指定（這一節是這個功能的核心）

一段對話由這個物件決定，它同時是送到 `/api/chat` 的東西：

```js
{
  scenario: {
    id: 'dialogue-1',                       // 或 'custom'
    title: '在咖啡店點餐',
    setting_zh: '你走進一家咖啡店，店員在櫃檯後面招呼你。',
    partnerRole_zh: '店員',                  // 對方演誰
    yourRole_zh: '顧客',                     // 你演誰
    goal_zh: '點一杯外帶拿鐵，問有沒有燕麥奶', // 選填：這一段要完成什麼
    category: 'travel',
  },
  level: 'tier-2',        // 沿用單字卡的難度分級
  partnerStyle: 'normal', // friendly | normal | brisk
}
```
...
| 來源 | 怎麼來 | 為什麼要有 |
|---|---|---|
| 現成的 93 段 | `content/dialogues.json` 每一段**已經有** `setting_zh`／`your_role_zh`／`partner_role_zh`／`category`／`difficulty` —— 直接當種子，一個欄位都不用補 | 一鍵就能開始。而且跟情境對話是同一批情境：腳本練完那一段，可以接著用自由發揮再走一次 |
| 八個情境的空白模板 | `lib/labels.js` 的 `CATEGORY_LABEL` 一個一個，各配一組預設角色 | 想在同一個情境裡換一個角色（同樣是職場，這次演面試官／演同事） |
| 自己寫 | 三個輸入框（情境、對方是誰、你是誰）＋ 選填的「這一段要完成什麼」 | 「明天要跟房東講電話」這種真實需求，腳本永遠不會有。**這一條才是這個模式真正的理由** |

幾個**刻意的決定**：

- **角色兩邊都要寫。** 只說「對方是店員」的話，模型不知道你是顧客、外送員還是
  來應徵的，第一句就會問錯問題。兩邊都寫的話，第一句台詞幾乎一定對。
- **`goal_zh` 是選填的，但填了差很多。** 有目標的對話會自己收尾（點到餐、換到貨、
  問到路），沒目標的會無限延伸下去 —— 而「這一段結束了」正是要給講評的時機。
  有 `goal_zh` 時，回應裡多帶一個 `goalMet: true/false`，前端就能提示「可以結束了」。
- **程度沿用單字卡的難度分級**（設定裡的 `vocabDeck`），不要再問一次 ——
  使用者已經選過「我是哪一級」了，再問一次等於承認我們沒在記。
  對照表（tier → 一句可執行的指示，例如「用國中程度的字，一句不超過 12 個字」）
  **寫死在 `server/chat.js` 一份**，不要在前端也放一份。
- **自己寫的情境會進 prompt**，所以它是使用者唯一能影響模型行為的入口 ——
  見雷 7，不是資安問題，是「會不會出戲」的問題。
- **不存 persona、不跨段記憶。** 每一段從情境開始、結束就結束（見非目標）。
...
```
前端  public/modes/chat.js        選情境與角色 → 對話串（打字送出）→ TTS 唸對方的話
                                  → 「結束這一段」→ 繁中講評
      public/lib/chat-scenarios.js 三個來源併成一份清單（純函式，測得到）
後端  server/chat.js              prompt 建構與回應解析（純函式）+ 呼叫（completeImpl 可注入）
      POST /api/chat              要登入。收 { scenario, level, messages }，回 { reply, goalMet }
      POST /api/chat/review       結束時一次要講評（階段 B）
```

`server/chat.js` **照 `server/coach.js` 的形狀寫**：那個檔案已經是
「純函式建 prompt（`buildReviewPrompt`）＋ 純函式解析回應（`parseReview`）＋
`completeImpl` 可注入的呼叫（`reviewAnswer`）」，所以 `coach.test.js` 不用金鑰就測得到。
聊天需要的東西一模一樣，不要另外發明一種。
...
| 能不能用，看 `/api/capabilities` | `aiReview`（`modelAvailability()`）已經是「有沒有一條真的能呼叫的模型」，聊天的條件一樣。**不要在前端自己從金鑰有沒有設定去推** —— 那個規則就會有兩份 |
...
| 一段對話一個 `sessionId`（前端生成） | 紀錄、講評、以及「這一段扣了幾次額度」都要指得回同一段 |
...
- `server/chat.js`
  - `buildChatPrompt({ scenario, level, messages })` 純函式：情境、兩邊的角色、
    程度指示、「只用英文、一到三句、不要幫使用者把話講完」的規則
  - `parseChatReply(raw)` 純函式：取出 `reply`，順便處理「模型回了一大段」
    「模型回了中文」「模型把自己的角色名寫在前面（`Clerk:`）」這三種情況
  - `chatReply(task, { completeImpl })` 照 `reviewAnswer()` 的形狀
- `public/lib/chat-scenarios.js`：把 93 段對話 + 八個情境模板 + 自訂的那一筆
  併成同一種形狀（純函式，餵真的 `dialogues.json` 測）
- `POST /api/chat`：要登入。上限：一句 500 字元、歷史 12 輪、單次 20 秒超時、
  自訂情境每個欄位 200 字元（跟講評同一個門檻，理由也一樣：這條路慢下來就是壞了）
- 沒設定金鑰時**卡片直接說「這個模式要金鑰」並指到設定頁** ——
  不要讓人打完一句送出才發現
- 前端：`public/modes/chat.js` + `lib/modes.js` 註冊（第九個分頁）+
  TTS 唸對方的話（`lib/tts.js` 現成）
- 驗收（`test/chat.test.js`，假的 `completeImpl`，不需要金鑰）：
  情境**與兩邊的角色**真的有進 prompt、程度換算成指示、歷史超過上限會截、
  供應商沒設定回的是「要設定」而不是 500、模型回空字串／回中文／回一大段時有退路
...
- 「結束這一段」→ 一次呼叫產出：3 條「你這樣說也可以，但更自然的是…」
  ...
  不然講評會變成罐頭建議
- 紀錄存 localStorage 的新鍵 `chats`，並加進 `backup.js` 的 `BACKUP_KEYS`
  與同步的鍵；合併規則照 `history` 那一條（取聯集 + 決定性全序），
  `merge.test.js` 要跟著加冪等與交換律那兩條
- `recordPractice('chat')` → **`storage.js` 的 `MODE_IDS` 要加 `'chat'`**
  （那份是「會累積每日進度」的清單，跟 `modes.js` 那份分頁的不一樣，兩份都要動；
  `vocabulary.test.js` 有一條在比對 `byMode` 的鍵，會自動跟上）
- `lib/settings.js` 的 `DEFAULTS.dailyGoals` 加一欄（單位是「段」），
  `modes/settings.js` 的 `GOAL_CHOICES` 也要加一組。
...
3. **額度的單位對不上。** `quota.js` 的預設是「每天 200 次」，而那個數字是照
   「一次動作一次呼叫」訂的 —— 聊天一段十輪就是十次，同一個數字的意思完全變了。
   **兩件事要一起做**：(a) 畫面上把額度講成「還可以聊幾段」而不是只有次數；
   (b) 開始一段之前就檢查「剩下的額度夠不夠走完一段」，不要讓人聊到第七句才被擋
   —— 那比一開始就擋還糟，因為前面六句的錢已經花掉了。
4. **對話紀錄裡有使用者自己打的內容。** 進備份與同步就等於上傳到伺服器
   —— 畫面上要講清楚，那跟「備份裡沒有金鑰」是同一個等級的承諾。
5. **離線。** PWA 的承諾是「離線也能練」，而這是唯一做不到的模式。
   模式列與首頁要看得出來，不能讓人打完一句才發現送不出去。
6. **模型會回中文、會回一大段、會自問自答。** prompt 要壓（「回一到三句、只用英文、
   **不要幫使用者把話講完**」），前端也要有退路 —— prompt 不是保證。
   「自問自答」是角色扮演最常見的壞掉方式：模型會一次把對方跟你的台詞都寫出來，
   所以 `parseChatReply()` 要看得出「回應裡出現了你的角色名」並且切掉。
7. **自訂情境是使用者唯一能影響模型行為的入口。** 這是自架自用，風險不是
   「injection 拿到別人的資料」（沒有別人的資料），而是「模型被帶去做別的事」
   —— 使用者自己要為此負責，但**系統提示不可以放在使用者能覆蓋的位置**，
   而且自訂欄位要有長度上限。
8. **`/api/chat` 不可以進 service worker 的快取**（跟金鑰、發音評估同一條理由：
   每次回應都不一樣）。`sw.js` 現在的規則已經涵蓋（其餘 `/api/*` 一律 network），
   但 `pwa.test.js` 的清單要加一條把它釘住。
9. **不要把它做成第二個情境對話。** 兩個模式會長得很像（都是對話串），
   但一個有參考答案、一個沒有。畫面上要一眼分得出來，不然使用者會問
   「為什麼這個沒有『對答案』」。

#### 什麼時候可以說「做完了」

- 階段 A：從三個來源各挑一個情境（現成的、模板的、自己寫的），
  都能來回五句以上而不出戲、不自問自答；沒設金鑰時畫面直接說要設定
...
### 能力量表的下一步：最近的走勢（**規劃，還沒實作**）

雷達圖回答「我現在哪裡弱」，回答不了「**我有沒有在進步**」—— 而那是練了兩個月
之後最想知道的一件事。四個面向各一條線，橫軸是日期。

**資料已經在了，不必再加一層。** `results` 留 400 天，四個面向的算法
（`lib/ability.js` 的 `computeAbility()`）本來就是「餵一段日子進去，得到四個分數」。
要做的是把「最近有練的 14 天」換成「逐日往回捲一次」。

#### 兩個真正要想清楚的設計問題

**一、一天一個點的話，那條線是雜訊不是走勢。**

一天的樣本很小 —— 聽力可能只有 11 題，一題就是 9 個百分點。畫成逐日正確率的話，
那條線會上下亂跳，而使用者會從隨機起伏裡讀出不存在的意義（「昨天怎麼突然掉了」）。

建議畫**滾動平均**：第 N 天那個點 = 「到第 N 天為止最近 7 個有練的日子」的分數，
也就是把 `computeAbility()` 的視窗往回滑。好處是它跟雷達圖上那個數字是同一套算法
（最右邊那個點就是雷達圖上的數字，兩張圖不會互相打臉）。
代價是**它反應得慢**，而畫面上要講清楚「這是最近 7 天的平均，不是那一天的成績」。

**二、沒練的那幾天怎麼畫。**

跟 `recentDays()` 面對的是同一個決定，但答案**不一定一樣**：

- 橫軸用**日曆天**：中間那段沒練的會變成一段平的線（滾動平均不變），
  看得出「這裡停了一週」—— 誠實，但一個月只練五天的人會看到一條幾乎全平的線
- 橫軸用**有練的日子**：每個點都有內容，但橫軸不再是時間，
  「上個月到這個月」這種話就講不出來了

先做日曆天（它才叫「走勢」），沒練的那幾天在圖上用淡色或斷點標出來。

#### 順手要處理的

`lib/trend-chart.js` **現在是跟讀專用的**，直接拿來用會出問題：

- 它吃的是 `{at, score, sentenceText}` 的逐筆紀錄，不是「四條線」
- `GRID_SCORES`（0 / 60 / 80 / 100）與 `trendCaption()` 的措辭（「最近 N 次的分數走勢」）
  都寫死了跟讀的假設
- 只畫一條線，沒有圖例

要嘛把它抽象成「吃 series 陣列」，要嘛另寫一個。**別直接改它然後只測新的那邊** ——
跟讀那張趨勢圖有 UI 測試（【3】），但那些測試驗的是「有沒有畫出來」，
不會告訴你圖的意思變了。

#### 放哪裡

首頁那張能力卡已經是整個首頁最高的一張了（手機上 2.3 個螢幕），
再疊一張走勢圖會把「今天的目標」推到看不到的地方。兩個選項：

- 能力卡上加一顆「看走勢」，進到一整頁（跟單字卡的「看複習盒」同一個做法）
- 走勢圖收在 `<details>` 裡，預設關著

前者比較好 —— 走勢是「偶爾回來看一次」的東西，不是每天要看的。

#### 什麼時候做

**等有兩三週的真實資料再做。** 聽力 / 中翻英 / 情境對話的成績是從能力量表上線
那天才開始累積的，現在做出來線上只有幾個點，看不出這張圖到底有沒有用 ——
而「門檻設多少、滾動幾天、沒練的那幾天怎麼畫」這三個決定，沒有真實資料就是瞎猜。

#### 什麼時候可以說「做完了」

- 四條線畫得出來，最右邊那個點跟雷達圖上的數字**對得起來**（不對就是兩套算法）
- 沒練的那幾天看得出來，而且不會被誤讀成「那天成績是 0」
- 圖底下有一句文字說明（讀螢幕的人看不到圖 —— `trendCaption()` 的規矩）
- 資料只有三、五天時不要畫成一條爛線，就說「再練幾天就看得到走勢」
...
已經還掉的（留著是因為「為什麼那樣做」還有用）：

- ~~**設定頁 10 個欄位擠在一起**~~ 已經分成卡片了（金鑰 / 講評端點 / 每日目標 /
  練習偏好 / 語音 / 同步 / 學習資料，寬螢幕上是網格）。

- ~~**每日目標是一個數字，複習與新字共用**~~ 拆了：設定 → 每日目標多一個
  「其中最多幾個新字」（預設 10），而且在今天的份裡**先保留名額**（最多佔一半，
  複習仍然優先）。規則在 `modes/vocabulary.js` 的 `startSession()`，
  排隊的純函式是 `storage.js` 的 `planQueue()`。
...
（四條規則改完之後剩下的。每一條都是**想清楚才沒做**，不是忘了。）

- **答錯的字，同一輪結尾補考一次。** 現在答錯會退一盒、`due` 設成「今天之內」，
  但隊伍在進來的時候就排好了，所以要等下一輪（按「再多練」、或明天）才會再出現。
  真正的間隔重複會在同一輪把它再問一次，答對才收工。改的位置是排隊那一層
  （`planQueue()` 之外再包一個「答錯的推回隊尾」），**不動盒子規則**。
- **間隔加 ±10% 的隨機抖動。** 一天發 10 個新字，35 天後那 10 個會同一天一起到期，
  一波一波地堆。抖動可以攤平，代價是「N 天後」這句話變成不完全準確 ——
  要嘛畫面改口，要嘛不做。
- **改版前在第 5 盒的字沒有自動升到第 6 盒**（它們從「已熟練」退回「學習中」，
  再答對一次就回去）。刻意不升：那等於憑空給一個沒答對過的間隔。
  真的要升的話**需要一個版本旗標**，因為它不是冪等的 —— 而 `migrateSrs()`
  整套設計就是「每次載入都跑、跑幾次都一樣」，不要為了這件事把那個性質弄破。
- **「建議升級」的門檻沒有跟著調。** `ADVANCE_MASTERED` 是「已熟練 ≥ 80%」，
  而「已熟練」現在要爬到第 6 盒 —— 同樣的練習量會比改版前晚一點看到那個提示。
  要不要補償，練一陣子有實際數字再說。
- **翻卡看不到「這張卡去了哪一盒」。** 選擇題答完會講（`boxNote()`），
  翻卡按了「記得／還不熟」直接換下一張。要嘛翻卡也停一下，要嘛就這樣 ——
  停一下等於多一次點擊，而翻卡的人本來就是要快。
...
- **首頁不載入任何字庫檔**。待複習的數字是直接數 `srs` 裡到期的筆數，
  ...
  能力量表只讀成績表、盒號與跟讀紀錄。`test/ability.test.js` 有一條測試
  直接掃 `lib/ability.js` 的原始碼擋這件事 —— 因為「為了算得更準而去 import 字庫」
  是每個人都會想做一次的事，而做了之後首頁會慢 3 MB 而且完全看不出原因。

### 能力量表與每日成績表

- **`results` 與 `activity` 是兩張表，不要合併。** `activity` 是連續天數與每日目標
  的唯一來源；`results` 只服務能力量表。混在一起的話，量表寫錯一行就會讓
  連續天數歸零 —— 而那是這個 App 裡最不該不小心弄丟的東西。
- **`results` 的計數器只能是「只會往上加」的東西。** 合併是逐格逐計數器取 max，
  存一個會上下跳的值（平均、最後一次的分數、比率）合出來會是一個**誰也沒看過的
  中間狀態**。要平均就存 `sum` 與 `n` 再相除。
- **加一個新的計數器要改三個地方**：`storage.js` 的 `RESULT_FIELDS`（白名單，
  不在裡面的欄位會被安靜丟掉）、記錄的那個模式、以及 `lib/ability.js` 裡讀它的地方。
  漏掉白名單的症狀最惡劣 —— 程式跑得好好的，只是那個數字永遠是 0。
- **樣本不夠時分數是 `null`，不是 0。** 畫面上要顯示「還沒練過」。
  畫成 0 等於對著沒碰過那一項的人說「你這裡是 0 分」。改門檻（`MIN_SAMPLE`）
  之前先想清楚：那是「一個數字開始有意義的最小量」，不是「多久才給看」。
- **總結要擋在「下一個」那顆按鈕上，不是在作答的當下。** 剛對完答案最想看的是
  自己這一句差在哪，總結蓋上去等於把那一題白答了。單字卡的 `!picked` 條件、
  跟讀的「一組練完」都是同一個坑，這次又踩了一次（第一版擋在 `recordAnswers()`）。
- **跟讀的兩張總結會撞在一起**：每日目標預設 5 句、一組也是 5 句。
  今天那張把一組那張吃掉，所以它補了「最高 / 最低」。動 `SET_SIZE` 或預設目標時
  記得這件事 —— 兩個數字不一樣之後，被吃掉的那次就不再涵蓋同一批句子了。
- **按鈕的標籤要跟著行為走。** 跟讀那顆「換一句」在有總結等著時會變成「看總結」，
  而**兩種總結都要算**（`summaryWaiting()`）。只算其中一種的話，按鈕上寫著
  「換一句」卻跳出一張總結 —— 那是使用者會認為壞掉的那種事。
- **`summarySeen` 刻意不進備份與同步。** 它是「這台裝置上這個畫面出現過沒」，
  不是學習進度。同步的話，在手機上看過總結，桌機打開就再也看不到那一天的總結了。
- **鼓勵只講事實，而且要是具體的數字。** 罐頭鼓勵（「太棒了！繼續加油！」）
  每天都一樣，看三次就變成一個要按掉的東西。`test/ability.test.js` 有一條
  掃過那幾句話確認裡面一個罰的字都沒有（「斷」「可惜」「白費」…）。
...
- **`BACKUP_KEYS` 漏加一個鍵 = 那種進度還原不回來**，而且完全沒有錯誤訊息。
  ...
  要進的話**還有三個地方**：`server/routes-auth.js` 的 `SYNC_KEYS`（不加的話
  伺服器會把它濾掉，備份還得回來但同步不會）、`lib/merge.js` 的 `mergeState()`
  （不加的話兩台裝置之間那份資料只有「先到的贏」）、以及 `test/merge.test.js`
  的隨機測試（新形狀要生得出來才掃得到邊界）。
- **`backupSummary()` 要讀得懂計數表的兩種形狀。** 它曾經只認得舊的
  「一天一個數字」，所以對任何一份現在的備份都回「每日紀錄 0 天」——
  而那句話正是「確定要用這份覆蓋嗎」要給的資訊。沒被抓到的原因是
  `test/backup.test.js` 的 fixture 也還是舊形狀。
