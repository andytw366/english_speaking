...
| 💬 情境對話 | 93 段 / 651 句台詞（八個情境）＋每日進度 | 完成 |
| 🗣️ 跟讀 | 2,041 句 / 8 種情境，Azure 逐音素評分 + 間隔重複 + 弱點音加權 + 連續天數 | 完成 |
| ⚙️ 設定 | 金鑰與**講評端點都能從網頁設**（只有擁有者，存在 `DATA_DIR/settings.env`）、中文講評開關、model、練習範圍、語音、學習資料、**跨裝置同步** | 完成 |
| 🔐 帳號 | 全部 `/api` 都要登入；進度存在伺服器上，**跨裝置自動合併**（手動整包覆蓋留著當逃生門） | 完成 |
| 🤖 LLM 即時對話 | 沒有腳本的自由對話 —— **只有規劃，還沒實作**（見「第四階段」） | 規劃 |

驗證狀態：`npm test` 387 項全過、`npm run test:ui` 238 項全過、
