...
| 🗂️ 單字卡 | 10,040 字，**自己選難度**（6 級）＋各級進度＋**每日目標**＋**中英雙向選擇題**（答完會列出其他選項的意思與發音），Leitner 盒子制＋**看複習盒**（哪些字在哪個盒子） | 完成 |
| ⌨️ 鍵盤 | 六個模式都能不摸滑鼠練完，**一套共通的語言**（Enter／N／P／S／A／H／T）＋ 全域的 `?` `[` `]`（提示在左側模式列，完整的一張表按 `?`） | 完成 |
| 📱 PWA | 加得到主畫面、斷線也打得開 | 完成 |
| 🎧 聽力 | 129 組 / 366 題（八個情境）＋每日進度（**照題組算，不是照題數**） | 完成 |
| ✍️ 中翻英 | **2,159 題**（填空 161 / 整句 1,998）＋每日進度＋**AI 修正**（跟情境對話同一份，**排在參考答案上面**） | 完成 |
| 💬 情境對話 | 93 段 / 651 句台詞（八個情境）＋每日進度＋**AI 修正你寫的那一句**（選用，走 `NARRATION_*` 那條路；結果卡最上面是 🤖／📘 兩句並列） | 完成 |
| 🗣️ 跟讀 | 2,041 句 / 8 種情境，Azure 逐音素評分 + 間隔重複 + 弱點音加權 + 連續天數 | 完成 |
| ⚙️ 設定 | **AI 的三張卡排在一起**（功能／金鑰與模型／上限，金鑰收在摺疊裡），三個 AI 功能用同一組選項（自動／手動／關）；金鑰與模型都能從網頁設（只有擁有者，存在 `DATA_DIR/settings.env`）；每日目標、練習範圍、語音、學習資料、**跨裝置同步**。**小字只留一句話**，細節指到說明頁 | 完成 |
| ❓ 說明 | 五張卡、21 組 `<details>` 問答（開始用／AI／進度與資料／怪怪的時候／鍵盤）—— 設定頁與練習畫面卸下來的那些字的去處 | 完成 |
...
最新的一件：**單字卡的複習規則改了四條**（規則與理由都在 README「🗂️ 單字卡」，
純函式在 `lib/storage.js`，`vocabulary.test.js` 與 `ui.mjs`【13b】逐條釘住）：

1. **到期以「日」為單位**（`dueAfterDays()`）。原本存的是「現在 +N×24 小時」，
   所以今晚 23:00 答的字明天整個白天都還沒到期 —— 只要今天比昨天早一點打開 App，
   昨天的字就一個都不算到期，而畫面上完全看不出原因。四條裡最有感的就是這條。
2. **答錯退一盒**，不是掉回第 1 盒。連錯兩次自然還是會退到第 1 盒。
3. **六個盒子**：1 → 3 → 7 → 16 → 35 天（原本 1 → 3 → 7 → 21）。
   「已熟練」的門檻跟著變成第 6 盒 —— 改版前在第 5 盒的字會退回「學習中」，
   再答對一次就回去。**刻意不自動升級**：那等於憑空給一個沒答對過的間隔。
4. **新字有自己的每日上限**（設定 → 每日目標 → 「其中最多幾個新字」，預設 10），
   而且在今天的份裡**先保留名額**（最多佔一半，複習仍然優先）。到期的字一多就
   連續好幾天看不到新字、到期是 0 的那天又灌進一整天份新字 —— 兩種都不好，
   因為今天發多少新字，明天就要回來複習多少。

順手修掉的一個 bug：**今天的最後一題答完會直接跳「今天練完了」**，
正確答案、卡片背面與「其他選項」整個被蓋掉。成績是在選下去的當下就記的，
所以答完最後一題時 `remaining` 立刻變 0 —— 完成卡多一個「這張卡還沒按下一題」
的條件就好。按鈕也從「下一題」改成「完成今天的份」。

再前一件：**帳號與跨裝置同步的階段 B 做完了 —— 同步現在是自動的**
...
| 模式 | 之前 | 現在 |
|---|---|---|
| 中翻英 | 279 題 | 2,159 題（腳本從 Tatoeba 匯入） |
| 聽力 | 81 組 / 226 題，**四個情境** | 129 組 / 366 題，**八個情境**（最少 10 組） |
| 情境對話 | 61 段，**四個情境** | 93 段 / 651 句，**八個情境**（最少 8 段） |

原本餐飲、購物、健康、學習在聽力與對話裡各 0 筆 —— 因為
`scripts/generate-content.mjs` 自己寫死了四個情境，而不是用 App 那八個。
設定頁那八顆按鈕照樣點得下去，點了會**靜靜退回全部題目**
（`listening.js` 的 `if (items.length === 0) items = raw`），所以一直沒被發現。

補的方式：**沒有金鑰，所以那 80 筆是手寫的**，但一樣走同一道門 ——
`--from <file>` 讀一批寫好的 JSON（不呼叫 API），照樣過 validate、去重、接 id。
那道門退掉了 6 筆「解析只是把英文原句抄一遍加中文句號」，正是下面「內容」
那一段記著反覆犯的錯。

還想再加的話：

```bash
node scripts/generate-content.mjs listening --plan     # 現況、缺哪些情境、建議指令
node scripts/generate-content.mjs dialogue  --plan
# 有金鑰時：
node scripts/generate-content.mjs listening --count 10 --category travel
# 沒金鑰、自己寫好一批：
node scripts/generate-content.mjs listening --from batch.json --dry-run
```

一定要跑過那支腳本的驗證，別手寫繞過去（見下面「內容」那一段的雷）。
驗收規則本身有測試（`test/content-generation.test.js`，27 條）：壞資料要被退掉、
重複要擋掉、**現有的每一筆都要過得了同一套規則**、每個情境至少 5 筆、
以及 keywords 對不上 accept 的數量不准再往上加（既有欠債 132 個，見下面）。

#### 為什麼不從現成語料匯入（查過了，2026-09）

「中翻英能離線補、聽力與對話只能生成」這個結論查證過，補上細節免得再問一次。

**這個開發容器連不出去。** 實測（`000` = 連不到）：

| 來源 | 結果 |
|---|---|
| `downloads.tatoeba.org` | 000 |
| `learningenglish.voanews.com` | 000 |
| `en.wikivoyage.org` | 000 |
| `huggingface.co` | 000 |
| npm registry / `raw.githubusercontent.com` / Gemini API | 通 |

所以「跑一支匯入腳本」在容器裡做不到，**除非那份語料包成 npm 套件**
—— 中翻英那批就是這樣進來的（`tatoeba-sentence-pairs-in-mandarin-chinese-english`，
7 MB，devDependency）。npm 上搜過 dialogue / subtitle / conversation / corpus /
phrasebook / esl / toeic，沒有一個是語料，全是 UI 的 dialog 元件與 OpenSubtitles 的
API wrapper。

**更根本的問題：外面找得到的是「文本」，這兩個模式要的是「題目結構」。**
用現有資料量過（中文以 2 個字元計）：

| | 原文佔內容量 | 剩下的是什麼 |
|---|---|---|
| 聽力 | 逐字稿 **30%** | 四個選項 + 繁中解析 |
| 情境對話 | 對方台詞 **21%** | 中文意圖、參考答案、accept 變體、keywords、用字說明 |

沒有任何語料有後面那七、八成。而且找來的文本常常**問不出題** —— 聽力題要有
可查證的細節（時間、數字、否定、地點），語料裡隨便一段話往往沒有；自己寫
（或叫模型寫）反而可以照著「這一組要練哪個聽力技能」設計。

真要用外部素材，可行的只有這幾條，**而且都得在本機跑**（容器連不到）：

| 來源 | 能給什麼 | 問題 |
|---|---|---|
| VOA Learning English | 有逐字稿、字彙刻意簡化，美國聯邦政府作品在美國不受著作權保護（17 U.S.C. §105） | 只解決 30% 那部分；夾帶的第三方照片與通訊社稿件不在其中 |
| Tatoeba（已在用，CC BY） | 更多中翻英 | 是單句句對，拼不出逐字稿或整段對話 |
| DailyDialog（約 13k 段日常對話） | 對話原文 | 授權是 CC BY-NC-SA：非商業 + 相同方式分享，而這個 repo 是公開的，衍生內容會被那個條款綁住 |
| OpenSubtitles / 電影對白 | 大量對白 | 口語破碎、內容不適合教學，授權也麻煩 |

**結論**：素材不是瓶頸，題目結構才是。要擴充就用金鑰跑
`scripts/generate-content.mjs`（`--plan` 會算出還缺多少、印出指令），
沒金鑰就自己寫一批再用 `--from` 過同一道門。
...
比下面那個「跟 LLM 即時對話」小得多，但它先解決了同一個痛點的一半：
**本地批改看不懂你寫了什麼**（關鍵字都有但文法壞掉會判「意思對了」）。
按「對答案」之後多一段模型看你那一句的修正 —— 教材的參考答案照樣在，
兩者回答的是不同的問題。設計與踩到的坑寫在 README「情境對話的 AI 修正」。

做的時候順手把兩件事抽出來，第四階段可以直接用：

- `narrator.js` 多了 `complete(prompt)` 與 `modelAvailability()` ——
  「現在接得到哪個模型」的邏輯仍然只有那一份。聊天要問的是同一個問題
- `openai-narrator.js` 的呼叫拆成 `completeViaOpenAI(prompt)`（原始文字）
  ＋ 一層薄的講評整理。prompt 由呼叫端組，請求形狀／超時／錯誤碼只有一份

後來補的兩件事（第四階段可以直接沿用）：

- **修正會存下來**（`localStorage` 的 `reviews`，也進備份與同步）。它同時是
  「同一句話不要付第二次錢」的快取 —— 句子一樣就直接拿舊的，不再呼叫
- **每天的呼叫上限**（`AI_DAILY_LIMIT` / `AI_DAILY_LIMITS`，預設 200 次）。
  所有模式、所有服務加在一起算一個預算，各個模型還可以再有自己的上限。
  計數存在伺服器（`DATA_DIR/usage.json`），不是瀏覽器 —— 存瀏覽器的話
  換裝置就重新開始。詳見 README「每天最多呼叫幾次」

聊天那邊要接上去的只有一件事：`spendQuota()` 的呼叫點（`server/index.js`）
—— 規則與計數都不必再寫一份。

**後來又做了三件事**（同一條線）：

1. **中翻英也有 AI 修正**，而且跟情境對話**共用同一份**（`server/coach.js` +
   `public/lib/ai-review.js`）。`mode` 決定上下文怎麼組，輸出格式與解析只有一份
2. **三個 AI 功能收成同一組選項**（自動／手動／關），設定頁上三張 AI 的卡排在一起
   （金鑰與模型合成一張，三組金鑰收在摺疊裡）。
   講評的手動模式為此多了 `POST /api/narration`（收算好的分數，不重送錄音）
3. **鍵盤補齊並統一**：Enter／N／P／S／A／H／T 在每個模式都是同一類的事，
   加上全域的 `?`（快捷鍵表）與 `[` `]`（換模式）

第四階段要的東西又少了幾樣：聊天可以直接沿用 `createReviewer()` 的狀態機
（token 作廢、快取、五種狀態各自的說法）與 `aiMode('chat')` 那一組設定。

**再後來又做了兩件事**（把「以 AI 為主」這件事在畫面上落實）：

4. **AI 改的那句排到參考答案上面**，兩句並列（`public/lib/answer-lines.js`，
   中翻英與情境對話共用）。原本 AI 那段接在整張結果卡的**最後面** ——
   要捲過逐字比對、其他說法、教材說明才看得到，而那些每一題都一樣。
   `createReviewer()` 因此從一個 `view()` 拆成 `render() → { row, notes, credit }`：
   row 進兩句並列、notes 接在下面、credit（哪個模型、等了幾秒）收進 `<details>`。
   聊天要用的話拆法也已經在了 —— 那邊同樣需要「一句話 + 說明 + 出處」三塊分開放
5. **小字大掃除 ＋ ❓ 說明頁**（`public/modes/help.js`）。規則寫在
   README「小字寫到什麼程度」：畫面上只留「現在要做這個決定時需要知道的事」，
   其餘搬到說明頁，而每張卡留一顆「看說明」指路。
   **按鈕會造成不可逆後果的那幾句一個字都沒縮**（覆蓋、清除、登出）

### 第四階段：跟 LLM 即時對話（**規劃，還沒實作**）

現有的 💬 情境對話是**腳本**：對方的台詞寫死，你講的每一句都有參考答案。
它練的是「這句話怎麼說」。真正缺的是**「對方回了你沒預期的東西，你怎麼接」**
—— 那件事腳本做不到，因為腳本只有一條路。
...
| 情境對話（現有） | 有標準答案的**一句** |
| 跟讀（現有） | 發音，逐音素的客觀分數 |
| **LLM 對話（新）** | 沒有標準答案的**一段** —— 反應速度、接話、追問 |

⚠️ **這個功能會改變 App 的成本結構。** 現在只有跟讀會花錢（送一次錄音 =
一次呼叫）。聊天是**每打一句就一次呼叫，而且要把前面的對話一起送上去** ——
所以下面的規劃裡「省錢」跟「好用」一樣重要。粗估一段十輪的對話大約
4～6k 個 input token 加幾百個 output（照你選的 model 的價目表換算），
而如果每一句都重送全部歷史，成本會隨句數**平方**成長。

#### 目標

1. **綁情境**，不做自由聊天 —— 沿用八個情境，可以拿現有 93 段對話的
   `setting_zh` 當種子（「你在藥局要買感冒藥，藥師會問你症狀」）
2. 對方的英文要**壓在使用者的程度**上。不壓的話學習者只會一直點頭
3. **講評事後給，不打斷** —— 打斷等於練不到流暢度，而且那是 N 倍的呼叫
4. 算進「今天」的進度 —— 會是**第六個會累積進度的模式**
   （`storage.js` 的 `MODE_IDS` 現在是五個），也是第八個分頁
5. **離線時要一眼看得出它不能用**，不是壞在半路

#### 非目標（刻意不做，寫下來免得之後有人「順手加」）

- **不做語音即時打斷（duplex）。** 那是完全不同量級的工程（串流 STT + 打斷偵測），
  而這個 App 的價值不在那裡
- **不做長期記憶的 persona。** 每一段對話從情境開始，結束就結束
- **不做自由聊天。** 沒有情境的話，模型會把話題帶去它最會講的地方，
  而使用者練到的英文範圍反而變窄
- **不做多人。**
- **金鑰不進瀏覽器。** 一律走後端，跟現在的設計一致（見 `server/settings.js`）

#### 形狀

```
前端  public/modes/chat.js   選情境 → 對話串（打字送出）→ TTS 唸對方的話
                             → 「結束這一段」→ 繁中講評
後端  server/chat.js         prompt 建構（純函式）+ 呼叫（fetchImpl 可注入）
      POST /api/chat         要登入。收 { scenario, messages }，回 { reply }
      POST /api/chat/review  結束時一次要講評（階段 B）
```
...
| 決定 | 為什麼 |
|---|---|
| 後端呼叫，前端只送文字 | 金鑰不能進瀏覽器 —— 那是 `settings.env` 整套設計的前提 |
| 供應商沿用 `narrator.js` 的那一份 | 「現在走哪一條路」的邏輯**只能有一份**（那個檔案開頭就寫著這件事）。聊天不是講評，但供應商的選法要共用 |
| 階段 A **不做 streaming** | 先量「一句等 2～4 秒」能不能接受。SSE 要對 Gemini 與 OpenAI 相容端點各寫一條，是第二步不是第一步 |
| 歷史只送最近 N 輪（N 先取 12） | 每句重送全部 = 成本平方成長。截的規則要能測 |
| 講評在「結束這一段」時一次要 | 每句都糾正會打斷流暢度，而且呼叫次數 ×N |
| 一開始只做打字 | STT 是另一組相依與延遲問題（階段 C）。打字版本已經在練「怎麼接話」了 |
| 每人每天有上限 | 見下面雷 3 —— 這是第一個「使用者打字就花錢」的端點 |

#### 階段切分

**階段 A：能對起來（文字）**

- `server/chat.js`：`buildChatPrompt({ scenario, level, messages })` 是純函式；
  呼叫走 `fetchImpl` 可注入的形狀（照 `server/openai-narrator.js` 抄）
- `POST /api/chat`：要登入。上限：一句 500 字元、歷史 12 輪、單次 20 秒超時
  （跟講評同一個門檻，理由也一樣：這條路慢下來就是壞了）
- 沒設定金鑰時**卡片直接說「這個模式要金鑰」並指到設定頁** ——
  不要讓人打完一句送出才發現。判斷的依據現成：`/api/capabilities` 的
  `narration.ready`（那就是「有沒有一條模型的路是通的」）；
  如果聊天允許的供應商跟講評不同，就在 capabilities 裡多回一個 `chat` 欄位，
  **不要在前端自己從金鑰有沒有設定去推**（那個規則就會有兩份）
- 前端：`public/modes/chat.js` + `lib/modes.js` 註冊 + TTS 唸對方的話
  （`lib/tts.js` 現成）
- 驗收（`test/chat.test.js`，假 fetch，不需要金鑰）：情境與程度真的有進 prompt、
  歷史超過上限會截、供應商沒設定回的是「要設定」而不是 500、
  模型回空字串或非預期格式時有退路

**階段 B：講評與紀錄（沒有這一段就只是聊天，不是練習）**

- 「結束這一段」→ 一次呼叫產出：3 條「你這樣說也可以，但更自然的是…」
  + 一句總評，繁中台灣用語。**prompt 要求引用使用者自己說過的句子**，
  不然講評會變成罐頭建議
- 紀錄存 localStorage 的新鍵 `chats`，並加進 `BACKUP_KEYS` 與 `SYNC_KEYS`；
  合併規則照 `history` 那一條（取聯集 + 決定性全序），
  `merge.test.js` 要跟著加冪等與交換律那兩條
- `recordPractice('chat')` → **`storage.js` 的 `MODE_IDS` 要加 `'chat'`**
  （那份是「會累積每日進度」的清單，跟 `modes.js` 那份七個分頁的不一樣，
  兩份都要動；`vocabulary.test.js` 有一條在比對 `byMode` 的鍵，會自動跟上）
- `lib/settings.js` 的 `DEFAULTS.dailyGoals` 加一欄（單位是「段」）。
  **不必寫 migrate** —— 沒設過的鍵會吃預設值，而 `dailyGoals` 是整包取新的
- 驗收：今天首頁與側欄的數字會動、備份帶得走、兩台裝置合併不會重複計
...
| 路 | 成本 | 問題 |
|---|---|---|
| 瀏覽器的 `SpeechRecognition` | 0 | 只有 Chrome 系列有、要連網、音訊會送到 Google |
| 後端用已經裝好的 Azure Speech SDK | 走 Azure 額度 | 要新端點 `/api/transcribe`：現有的 `/api/pronunciation-feedback` 是「對著參考句評分」，自由說話沒有參考句 |

順帶一提：Azure 那條路可以**每一句都拿到發音分數**（現有端點已經回
`recognizedText`），那會把這個模式跟跟讀接起來。但那是加分項，不是前提。

**階段 D：延遲（只有在 A 真的覺得慢的時候才做）**

SSE 串流。Gemini 與 OpenAI 相容端點的串流 API 不一樣，所以 `chat.js` 裡會分兩條，
測試各自餵假的串流。

#### 會踩的雷（先寫下來）

1. **供應商選擇不要再寫第二份。** `server/narrator.js` 的開頭就寫著為什麼
   —— 三個地方各判斷一次，一定會有一天「畫面說在用 A，實際跑的是 B」。
2. **歷史成本會平方成長。** 每句都重送全部對話的話，第十句要送九句的歷史。
   截，而且截的規則要有測試（不然「怎麼越聊越貴」查不出來）。
3. **rate limit 從階段 A 就要有。** 3-c 那條欠債（`/api/pronunciation-feedback`
   沒有次數上限）在聊天上會變嚴重：錄音要開口、打字不用。
   每人每天的上限比全域的上限重要（花的是擁有者的錢）。
4. **對話紀錄裡有使用者自己打的內容。** 進備份與同步就等於上傳到伺服器
   —— 畫面上要講清楚，那跟「備份裡沒有金鑰」是同一個等級的承諾。
5. **離線。** PWA 的承諾是「離線也能練」，而這是唯一做不到的模式。
   模式列與首頁要看得出來，不能讓人打完一句才發現送不出去。
6. **模型會回中文、會回一大段。** prompt 要壓（「回一到三句、只用英文」），
   前端也要有「太長就截」的退路 —— prompt 不是保證。
7. **使用者輸入會進 prompt。** 這是自架自用，風險不是「injection 拿到別人的資料」
   （沒有別人的資料），而是「模型被帶去做別的事」。不必過度設計，
   但別把系統提示放在使用者可以覆蓋的位置。
8. **`/api/chat` 不可以進 service worker 的快取**（跟金鑰、發音評估同一條理由：
   每次回應都不一樣）。`sw.js` 現在的規則已經涵蓋（其餘 `/api/*` 一律 network），
   但 `pwa.test.js` 的清單要加一條把它釘住。

#### 什麼時候可以說「做完了」

- 階段 A：選一個情境，能來回五句以上而不出戲，沒設金鑰時畫面直接說要設定
- 階段 B：結束一段會拿到引用自己句子的繁中講評；今天的數字會動；備份帶得走
- 之後才考慮 C／D

> **如果只是想要「有人陪我練」，階段 A 就夠用了。**
> B 之後才是「練完會進步」，而那才是這個 App 跟聊天機器人的差別。
...
- **`seed()` 會等 `#nav .tab` 可見，而手機寬度下側欄整個藏起來** ——
  縮到 390px 之後再 `seed()` 會一路 timeout 30 秒才炸，而錯誤訊息寫的是
  「resolved to 8 elements」（找得到、但不可見），完全看不出跟視窗寬度有關。
  **先 seed 再縮視窗**，而且縮完要記得縮回去，不然後面每一段都在手機寬度下跑。
- **加一個模式要改三支瀏覽器測試，而 `e2e.mjs` 是最容易漏的那一支**（它在本機
  要有麥克風才跑得動，所以常常只跑了 `test:ui`）。分頁數那條斷言因此寫錯過兩次
  ——「今天」那一頁做出來之後還寫著六、加了「說明」之後還寫著七，兩次都是 CI
  才抓到。**三支現在都讀 `MODE_IDS`**（`layout.mjs` 連清單都是），不要再寫死數字。
  順帶一提：`npm run test:e2e` 的【1】【2】【4】不需要金鑰，本機跑得起來 ——
  改動到外殼（`app.js` / `index.html` / `lib/modes.js`）時記得連它一起跑。
- **`viewText()` 讀的是 `textContent`，收起來的 `<details>` 裡的字也算**。
  所以「畫面上看得到 X」這種檢查在細節收進摺疊之後仍然會過 —— 要驗「排在
  上面」「預設收起來」得改看 DOM 順序與 `el.open`（結果卡那幾條就是這樣寫的）。
...
- **`keywords` 只保證在 `answer` 裡找得到，不保證在其他 `accept` 裡也找得到。**
  `grade()` 先比對 accept 完全相符（判「完全正確」），但**照著 accept[1] 改寫一下**
  就只剩 keywords 那條路 —— 使用者寫出畫面上列為「其他說法」的變化型，卻被判
  「再想想」。既有的 61 段有 132 個這種變體（新加的 32 段是 0 個），
  `content-generation.test.js` 用棘輪釘住「不准再往上加」。
  寫新內容時的做法：**keywords 挑兩邊 accept 都有的字**，而且注意單複數
  （socket / sockets 在 `tokens()` 眼裡是兩個字）與 thanks / thank you。
- **聽力的正解位置會不自覺地集中。** 366 題原本有 68% 的正解在第二個位置，
  自己新寫的 48 組也有 59% —— 所以選項改成在 `pick()` 的時候洗牌
  （`lib/practice.js` 的 `shuffleOptions()`）。寫題目時不必刻意分散，
  但**別把洗牌拿掉**。
