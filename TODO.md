...
| ✍️ 中翻英 | **2,159 題**（填空 161 / 整句 1,998）＋每日進度＋**AI 修正**（跟情境對話同一份） | 完成 |
| 💬 情境對話 | 93 段 / 651 句台詞（八個情境）＋每日進度＋**AI 修正你寫的那一句**（選用，走 `NARRATION_*` 那條路） | 完成 |
| 🗣️ 跟讀 | 2,041 句 / 8 種情境，Azure 逐音素評分 + 間隔重複 + 弱點音加權 + 連續天數 | 完成 |
| ⚙️ 設定 | **AI 的三張卡排在一起**（功能／金鑰與模型／上限，金鑰收在摺疊裡），三個 AI 功能用同一組選項（自動／手動／關）；金鑰與模型都能從網頁設（只有擁有者，存在 `DATA_DIR/settings.env`）；每日目標、練習範圍、語音、學習資料、**跨裝置同步** | 完成 |
| 🔐 帳號 | 全部 `/api` 都要登入；進度存在伺服器上，**跨裝置自動合併**（手動整包覆蓋留著當逃生門） | 完成 |
| 🤖 LLM 即時對話 | 沒有腳本的自由對話 —— **只有規劃，還沒實作**（見「第四階段」） | 規劃 |

驗證狀態：`npm test` 465 項全過、`npm run test:ui` 261 項全過、
...
（token 作廢、快取、四種狀態各自的說法）與 `aiMode('chat')` 那一組設定。
