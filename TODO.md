...
| ⚙️ 設定 | 金鑰與**講評端點都能從網頁設**（只有擁有者，存在 `DATA_DIR/settings.env`）、中文講評開關、**情境對話 AI 修正的自動／手動**、**每天的呼叫上限**、model、練習範圍、語音、學習資料、**跨裝置同步** | 完成 |
| 🔐 帳號 | 全部 `/api` 都要登入；進度存在伺服器上，**跨裝置自動合併**（手動整包覆蓋留著當逃生門） | 完成 |
| 🤖 LLM 即時對話 | 沒有腳本的自由對話 —— **只有規劃，還沒實作**（見「第四階段」） | 規劃 |

驗證狀態：`npm test` 448 項全過、`npm run test:ui` 243 項全過、
