...
驗證狀態：`npm test` 209 項全過、`npm run test:ui` 180 項全過、
...
最新的一件：**版面改版**（三個 commit）。桌機上原本只用掉 47% 的寬度、
...
**第一階段做完了。** 接下來是第二階段（鍵盤操作、釋義截斷、PWA）。

### 第二階段：日常手感

**2-a. 鍵盤操作** ← **下一步** —— 選擇題按 `1`–`4`、`Enter` 下一題、空白鍵翻卡。
現在只有中翻英有 Enter（`translation.js` 的 `onEnter`）。桌機練起來差很多。
版面改版之後更划算：桌機上一題已經整頁看得完，不必捲，剩下的就是不要摸滑鼠。
（左側模式列下方有空間可以放快捷鍵提示。）

**2-b. 釋義截斷** —— 6% 的字釋義超過 60 字（`go` 有 20 個義項、`make` 19 個），
答完題後那一坨會蓋掉重點。只顯示前幾個義項＋「看全部」。
**做了一半**：`lib/quiz.js` 的 `briefMeaning()` 已經在用（答完之後那三個選項的釋義截到
3 個義項），剩下的是**卡片背面**那個完整的 `meaning_zh`（`cardBack()`）——
那裡需要的是「前幾個義項＋一顆『看全部』」，因為背面本來就是要給看完整的。

**2-c. 手機（PWA）** —— **版面改版做掉了一半**：下方的模式列、觸控目標 44px、
窄螢幕的間距與字級都有了（見 README「手機」）。**剩下的是 PWA 本身** ——
manifest、圖示、service worker，以及 favicon（現在 console 一直有一筆 404，
`test/ui.mjs` 還特地繞過它）。加上去就能加到主畫面像 App 一樣開；
