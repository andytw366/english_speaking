...
| ⚙️ 設定 | **AI 的四張卡排在一起**（功能／金鑰／模型／上限），三個 AI 功能用同一組選項（自動／手動／關）；金鑰與模型都能從網頁設（只有擁有者，存在 `DATA_DIR/settings.env`）；每日目標、練習範圍、語音、學習資料、**跨裝置同步** | 完成 |
| 🔐 帳號 | 全部 `/api` 都要登入；進度存在伺服器上，**跨裝置自動合併**（手動整包覆蓋留著當逃生門） | 完成 |
| 🤖 LLM 即時對話 | 沒有腳本的自由對話 —— **只有規劃，還沒實作**（見「第四階段」） | 規劃 |

驗證狀態：`npm test` 465 項全過、`npm run test:ui` 257 項全過、
...
2. **三個 AI 功能收成同一組選項**（自動／手動／關），設定頁上四張 AI 的卡排在一起。
