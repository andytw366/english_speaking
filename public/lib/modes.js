...
export const MODES = [
  ...
  // 說明放在最後，而且**不進下方那一列**（跟設定一樣走頁首的按鈕）——
  // 手機那一列放得下五顆，六顆就開始擠成兩行。
  //
  // 為什麼要有這一頁：設定頁與各練習畫面原本背著整份說明書，
  // 而那些字每次進去都在那裡（第一次有用，第二十次是噪音）。
  // 現在畫面上只留「這一刻要做決定時需要知道的事」，其餘搬到這裡。
  {
    id: 'help',
    label: '說明',
    icon: '❓',
    subtitle: '常見問題 —— 不用讀，需要的時候再來翻。',
  },
];
