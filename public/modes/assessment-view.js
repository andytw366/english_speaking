 ...
 *   reference 是範例句的曲線（`lib/reference-pitch.js`），有就在圖上疊第二條
 */
export function renderAssessment(container, data, targetText, replaceSentence, options = {}) {
  ...
  append(container, pitchFigure(options.pitch, data.words, options.reference));
 ...
 */
function pitchFigure(contour, words, reference) {
  if (!contour) return null;
  const chart = buildPitchChart(contour, { words, reference, titleId: 'pitch-caption' });
  ...
  return h('figure', { class: 'pitch' },
    ...
    h('p', { class: 'hint' },
      '高低是相對於各自那一句的平均，斷開的地方是沒有聲音的子音或停頓。' +
      (reference ? '兩條線都對齊到你的節奏上，所以比的是形狀而不是快慢。' : '')),
  );
}
