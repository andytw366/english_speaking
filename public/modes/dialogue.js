...
function render() {
  ...
  root.append(transcriptCard());

  if (isFinished()) {
    root.append(summaryCard());
  } else if (currentTurn().speaker === 'partner') {
    root.append(partnerCard());
  } else {
    root.append(yourTurnCard());
...
function transcriptCard() {
  ...
  if (shown.length === 0) {
    card.append(h('p', { class: 'hint' }, '對話還沒開始。'));
    return card;
  }
...
function resultCard() {
  ...
  if (level === 'close') {
    card.append(h('p', { class: 'hint' }, '關鍵用字都有，說法跟參考答案不同沒關係。'));
  }
  ...
  if (level !== 'exact') {
    card.append(diffView(checked.input || '（空白）', turn.answer));
  }

  if (turn.accept.length > 1) {
    card.append(h('p', { class: 'hint' }, `另一種說法：${turn.accept[1]}`));
  }
  card.append(h('p', { class: 'explain explain--neutral' }, turn.note_zh));

  // 知道正確說法之後再練發音才有意義，所以錄音放在這裡而不是作答前
  if (recSupported()) card.append(recordingRow(turn.answer));
