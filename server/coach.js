...
// 用了比教材那句更自然的說法反而可能判「再想想」。
//
// 所以**回來的判定就是那一題的判定**（結果卡最上面那一句，見
// `public/lib/verdict.js`），教材附的那句話降級成一個例句：它是「可以這樣說」
// 的一種寫法，不是標準答案。模型沒回來的時候才退回本地那三級 ——
// 那時候畫面上確實只有例句可以比。
//
// 這件事會回頭影響 prompt：模型不知道自己的判定會被當成結論的話，它會傾向
// 「跟參考答案不一樣就是要改」，而那正是本地比對會犯的錯。所以下面的規則裡
// 明講了教材那句只是其中一種說法。
//
// ─── 為什麼兩個模式共用一份 ──────────────────────────────────────────────
//
 ...
 * - **教材那句要進去，但要明講它只是一個例句、而且不要直接抄。** 不給的話
 *   模型會自己想一句，跟教材的說法對不上；給了而不講清楚，回來的就永遠是
 *   那一句本身 —— 而那個畫面上已經有了，使用者要知道的是**自己那句**行不行。
 *   更重要的是**判定**：不講的話，模型會拿它當標準答案，然後把每一種
 *   跟它不同的說法都判成「要改」——那正是本地關鍵字比對會犯的錯，
 *   而現在判定是模型說了算。
...
export function buildReviewPrompt(task = {}) {
  ...
  // 其他例句只列跟主要那句不同的 —— accept[0] 通常就是 reference 本身，
  // 重複列一次只是把 prompt 撐長
  const others = accept
  ...
  const lines = [
    ...
    others.length ? `教材附的其他例句：${others.map((a) => `「${a}」`).join('、')}` : '',
    '',
    `學習者實際寫的是：「${input}」`,
    '',
    ...
    '',
    '規則：',
    '- 教材附的那句話**只是一個例句，不是標準答案**。' +
      '同一個意思有很多種講法 —— 學習者寫得跟它不一樣（甚至比它更自然）都不是問題，' +
      '**只有他自己那一句本身有問題的時候才算有問題**。判定的是他寫的那一句。',
    '- 「修正」那一句要**盡量貼近學習者原本的說法，只改必要的地方**。' +
      '不要把教材的參考說法整句抄過來 —— 那一句他已經看得到了，' +
      '他要知道的是「我自己這樣講行不行」。原句已經自然的話就原句照抄。',
    mode === 'translation'
      ? '- 判定的標準：意思與中文相符、文法與用字自然＝可以（**跟例句不同也算可以**）；' +
        '意思到了但不夠道地或有小錯＝小問題；意思跟中文不一樣、或文法錯到會被誤解＝要改。'
      : '- 判定的標準：文法與用字在這個情境下都自然＝可以（**跟例句不同也算可以**）；' +
        '意思通但不夠道地或有小錯＝小問題；文法錯到會被誤解、或答非所問＝要改。',
...
function dialogueContext(task) {
  ...
  return [
    ...
    reference && `教材附的一句例句（只是其中一種說法）：「${reference}」`,
  ];
}

/**
 * 中翻英的上下文：中文題目、題型、教材附的例句。
 *
...
function translationContext(task) {
  ...
  return [
    ...
    isCloze
      ...
      : '題型：整句翻譯。',
    reference && (isCloze
      ? `教材填在空格裡的字（只是其中一種填法）：「${reference}」`
      : `教材附的一句例句（只是其中一種譯法）：「${reference}」`),
  ];
}
